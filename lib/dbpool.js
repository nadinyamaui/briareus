    ...
    if (free) {
      ...
      try {
        ...
        if (project.dbRestoreSql) {
          // Whatever a previous session's profiles left here goes before the
          // restore (see ensureProfileDatabase): a release drops them, but a
          // restart or a crash lets go of the claim without one.
          const profileDbs = poolProfileDatabases(project);
          if (profileDbs.length) {
            onEvent(
              `Dropping the run profiles' databases ${profileDbs.join(', ')} on ${free.host}:${free.port}, so they start over…`,
            );
          }
          for (const database of profileDbs) await dropDatabase(free, database);
          onEvent(
...
// server.
function poolProfileDatabases(project) {
  const names = new Set();
  for (const profile of projectRunProfiles(project)) {
    const value = profile.env.find(([key]) => key === 'DB_DATABASE')?.[1];
    if (!value) continue;
    const name = render(value, { database: project.dbPoolDatabase, profile: profile.name });
 ...
 * has to go the same way, or the next session would: it is dropped when the
 * server is released, and the next claim's restore (a merged dump that holds
 * it) or the next CREATE starts it over. The claim drops the names the
 * profiles render to as well, before its restore, since a restart or a crash
 * lets go of a server without any release. Without a dump the pooled database
 * stays as it is, and so does this one.
 *
...
export async function ensureProfileDatabase(job, database, onEvent = () => {}) {
  ...
  const server =
    job.dbServerId != null ? getDbServer(job.dbServerId) : job.sessionDb ? templateDb(project) : null;
  if (!server) {
    // A local checkout's database is the developer's own business: nothing
    // here knows which server it lives on, let alone how to reach it.
    if (job.local) {
      onEvent(
        `This checkout's databases are not managed here, so the run profile's database ${database} is not created: create it yourself if it does not exist yet.`,
      );
    }
    return false;
  }
...
// The run profiles' databases on the server go first (see
// ensureProfileDatabase), and the claim is only handed back once they have:
// a session claiming it in between would have its restore dropped from under
// it. A drop that fails is let go of, like dropSessionDatabase's: the cost is
// a database the next session finds as it was, the same as before the drop.
export async function releaseInstance(job) {
  ...
  const server = profileDbs.length ? getDbServer(serverId) : null;
  try {
    for (const database of server ? profileDbs : []) {
      await dropDatabase(server, database).catch((e) =>
        console.error(`could not drop ${database} on ${server.host}:${server.port}:`, e.message),
      );
    }
  } finally {
