 ...
 * (profiles name it off {database}), so it is remembered and dropped with it.
 * On a claimed server it stays, like the pooled database does.
 *
...
export async function ensureProfileDatabase(job, database, onEvent = () => {}) {
  ...
  if (job.dbServerId == null && !(job.profileDbs || []).includes(database)) {
...
export async function dropSessionDatabase(job, onEvent = () => {}) {
  ...
  // drop stays on the list, for the next close to try.
  for (const profileDb of job.profileDbs || []) {
    try {
