 ...
 * it) or the next CREATE starts it over. Without a dump the pooled database
...
export async function ensureProfileDatabase(job, database, onEvent = () => {}) {
  ...
  if (job.dbServerId == null && ownsProfileDb(job, database) && !(job.profileDbs || []).includes(database)) {
...
export async function dropSessionDatabase(job, onEvent = () => {}) {
  ...
  // drop stays on the list, and the session's own database stays with it: the
  // name they are both built off is what lets the next close (or the delete)
  ...
  if (leftover) {
    onEvent(`Kept this session's database ${database} for now, so the next close tries those again.`);
    return false;
  }
  try {
