...
// The engine a DB_CONNECTION value speaks: Laravel's driver names, and what a
// .env commonly writes for them. Anything else (a connection named in the
// app's config/database.php, such as `central`) is returned as typed.
...
// from a .env does, is not another server. DB_CONNECTION is a connection name
// in Laravel, so the one the .env template gives the session (`central` in a
// multi-tenant app) is its own too, next to the server's engine.
