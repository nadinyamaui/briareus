...
// its single state is split into the two. Takes the commit's statusCheckRollup;
// the session sync in lib/jobs.js reads the same shape. A check run links to
// the integrator's own page (detailsUrl) when it set one, and otherwise to its
// page on GitHub (url), which every run has.
