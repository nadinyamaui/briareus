...
// Conclusions the panel and the in-app viewer both count as failed. Cancelled,
// stale, skipped, and the rest stay off this list so a cancelled run does not
// show as a red failure in one place and a non-failure in the other.
export const FAILED_CHECK_CONCLUSIONS = ['failure', 'timed_out', 'action_required'];

export function checkFailed(conclusion) {
  return FAILED_CHECK_CONCLUSIONS.includes(conclusion);
}

// A legacy commit status has one state; the row shape the panel draws splits
// it into status + conclusion. PENDING/EXPECTED (and REST's lowercase forms)
// are still running; anything other than success is a failure.
function commitStatusRow(state, name, url) {
  ...
  return {
    ...
    url: url || null,
    // Shown with the check runs, but not counted in the CI verdict the
    // session sync hands an orchestrator (ciChecks in lib/jobs.js).
    commitStatus: true,
  };
}
...
// the panel draws. Takes the commit's statusCheckRollup; the session sync in
// lib/jobs.js reads the same shape. A check run links to the integrator's own
// page (detailsUrl) when it set one, and otherwise to its page on GitHub
// (url), which every run has.
  ...
  return nodes.map((n) => {
    if (n.__typename === 'CheckRun')
      return {
        ...
        // The run's identity, where the query asked for it (the session sync
        // does, to tell a re-run from the run it replaced).
...
// The panel's commit rows: sha, first message line and a link each. Takes the
// `commits { nodes { commit { ... } } }` nodes; the session sync reads the same.
...
// The issues a pull request closes, as the panel lists them: its "Development"
// links on GitHub. Takes the closingIssuesReferences nodes.
...
// One standing verdict per reviewer, folded chronologically: a real verdict
...
// just comments), and a dismissal clears the standing verdict. The session
// sync in lib/jobs.js folds its reviews through this too.
