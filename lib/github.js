...
// Removing a label is idempotent from the dashboard's point of view: GitHub
// answers 404 when the pull request did not carry it, which is already the
// requested end state. Every other refusal is actionable and must surface.
...
// Conditional GETs. GitHub tags every REST answer with an ETag and, asked
// with If-None-Match, answers an unchanged resource with a 304 that does not
// count against the rate limit. The sync tick reads the same pull requests
// over and over, and nearly every read finds nothing new, so remembering the
// tag and the body it came with turns most of that traffic into free calls.
// Opt-in per call (`{ conditional: true }`): only the URLs a timer polls are
// worth holding a body for, and everything read once would only fill the map.
// Keyed per token and URL: a tag earned by one token says nothing to another.
...
// The response handed back when the cache took part: the same shape the
// callers read (ok, status, headers, json) with `notModified` saying whether
// GitHub confirmed the cached body rather than sending a new one, and `etag`
// the tag the body carries, for a caller that has to know whether the resource
// changed since *its* last read rather than since anybody's. Every json() is a
// copy of its own, so a caller that edits the answer edits nobody else's.
...
export async function githubRest(cfg, method, url, body, { conditional = false } = {}) {
  checkCooldown('core');
...
// Whose account the app is acting as. The token's own user, so the app can
// tell a pull request the user opened from somebody else's without a username
// having to be typed into the project's settings. Cached per token: it is one
// question with one answer for the life of a token, asked on a path (holding a
// review's findings) that should not spend a round trip on it every time.
...
export async function viewerLogin(cfg) {
  ...
  try {
    ...
    // A token that cannot answer this leaves the question open rather than
    // failing whatever asked; the caller decides what an unknown viewer means.
  }
  // A failed lookup is cached too, until the token changes: the alternative is
  // a doomed request on every review that ends.
