...
// GitHub confirmed the cached body rather than sending a new one.
function cachedResponse(res, body, notModified) {
  return {
    ok: true,
    status: 200,
    headers: res.headers,
...
export async function githubRest(cfg, method, url, body) {
  checkCooldown('core');
  const cacheKey = method === 'GET' ? `${cfg.githubToken}\n${url}` : null;
  ...
  if (cached && res.status === 304) {
    ...
    return cachedResponse(res, cached.body, true);
  }
  if (cacheKey && res.ok) {
    ...
    if (etag) {
      ...
      return cachedResponse(res, fresh, false);
    }
  }
  return res;
}
