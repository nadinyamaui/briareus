...
function addCalibrationRow(rows, row) {
  if (!(row.costUsd > 0) || !(row.inputTokens > 0)) return rows;
  const found = rows.findIndex(
    (candidate) => candidate.provider === row.provider && candidate.model === row.model,
  );
  const previous = found === -1 ? null : rows[found];
  const next = {
    provider: row.provider,
    model: row.model,
    inputTokens: (previous?.inputTokens || 0) + row.inputTokens,
    outputTokens: (previous?.outputTokens || 0) + (row.outputTokens || 0),
    costUsd: (previous?.costUsd || 0) + row.costUsd,
  };
  if (found === -1) return [...rows, next];
  return rows.map((candidate, index) => (index === found ? next : candidate));
}
    ...
    saveTurnUsage(row)
      .then(() => {
        if (row.costUsd > 0 && row.inputTokens > 0) {
          calibrationRevision++;
          if (calibrationRows) calibrationRows = addCalibrationRow(calibrationRows, row);
        }
      })
