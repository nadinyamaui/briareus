...
export function filterUsageRows(rows, projects = [], filter = {}) {
  ...
  return rows.filter(
    (r) =>
      ...
      matches(filter.pricing, r.costUsd == null ? 'unpriced' : r.costEstimated ? 'estimated' : 'reported'),
  );
}
...
// Coarser than modelUsage: one CLI's spend spans its models and accounts. A row whose provider was never recorded
...
export async function overallUsage(projects, period = 'month', now = Date.now(), filter = {}) {
  ...
  return {
    ...
    models: modelUsage(rows),
    activities: activityUsage(rows),
    monthlyTotal: window.period === 'month' ? aggregateUsage(all) : null,
