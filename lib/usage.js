// @ts-check
import { saveTurnUsage, loadTurnUsage, loadAllTurnUsage, loadJobTurnUsage } from './db.js';
...
// project dashboards without rewriting that source data. Rows stay attached
// so the transcript can put each estimate on its own turn footer too.
export async function jobUsageEstimates(jobIds, now = Date.now()) {
  const rows = await estimateCosts(await loadJobTurnUsage(jobIds), now);
