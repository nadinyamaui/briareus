...
// project dashboards without rewriting that source data. The whole ledger
// calibrates pricing so selecting one session, every session, or a deletion
// subtree cannot change a turn's estimate. Requested rows stay attached so the
// transcript can put each estimate on its own turn footer too.
export async function jobUsageEstimates(jobIds, now = Date.now()) {
  const requested = new Set((jobIds || []).filter(Boolean));
  const rows = await estimateCosts(await loadAllTurnUsage(), now);
...
export async function recordTurnUsage(job, turn, provider, model) {
  ...
  if (!row) return;
  try {
    await saveTurnUsage(row);
  } catch (e) {
    console.error(`turn usage not recorded for ${job.id}: ${e.message}`);
  }
}
