...
// The two keys the dashboard's pickers filter on.
//
// A project's key names the group projectBreakdown puts its rows in — the
// stable id while a Settings row still claims them, the repository otherwise —
// so a filter and the table it was picked from can never disagree about which
// rows are "that project". A model's key names the provider as well, since two
// providers can sell the same model id and the ledger already prices and
// groups them apart.
...
// Which configured project owns a row, by the rule projectBreakdown documents
// below. Built once per call because both the grouping and the filtering ask
// it of every row in the window.
...
// The rows one of the dashboard's picks leaves standing. A pick nothing
// matches (a model that only ran last month, a project whose turns are all
// outside this window) narrows to nothing rather than being ignored: an empty
// window is an answer, and quietly widening it back to everything would put a
// total on screen under a label that does not describe it.
export function filterUsageRows(rows, projects = [], { project = null, model = null } = {}) {
  if (!project && !model) return rows;
  const owner = projectFinder(projects);
  return rows.filter((r) => {
    if (project && projectFilterKey(owner(r), r.repo) !== project) return false;
    if (model && modelFilterKey(r.provider, r.model) !== model) return false;
    return true;
  });
}

// What the pickers offer, drawn from the whole window rather than from what is
// left after a pick: a filter must never remove itself, or the projects
// beside it, from the list it was chosen out of. Each option carries the
// tokens behind it so the picker can be ordered by spend, which is the order
// the tables under it are in.
...
// The same rows grouped by the CLI that ran them, biggest token spender first.
// Coarser than modelUsage on purpose: "what is codex costing me" is a question
// about an account and a subscription, and one provider's spend is spread over
// however many models it was asked for. A row whose provider was never recorded
// groups under null, which the UI names rather than guessing at.
...
export async function overallUsage(projects, period = 'month', now = Date.now(), filter = {}) {
  ...
  // The pickers are built from the whole window, the numbers from what the
  // pick leaves standing; see usageFilterOptions. Both keys are echoed back
  // because either can outlive what it named — a model retired mid-month, a
  // project deleted from Settings — and the page has to keep showing the
  // filter it is drawing under.
  const project = filter.project || null;
  const model = filter.model || null;
  const options = usageFilterOptions(all, projects);
  const rows = filterUsageRows(all, projects, { project, model });
  // Narrowed to one project, the by-project table is that one row. The rest of
  // the list would be every configured project at zero, which answers a
  // question nobody asked here: "which projects ran nothing?" is worth saying
  // over the whole window, not under a pick that excluded them by hand.
  const listed = project ? projects.filter((p) => projectFilterKey(p, p.repo) === project) : projects;
  ...
  return {
    ...
    models: modelUsage(rows),
    activities: activityUsage(rows),
    filter: { project, model },
    options,
  };
}

// Lifetime pricing details for the restored session records. The jobs table
// intentionally keeps the provider-reported total, so Codex's value there is
// null; this read-side projection adds the same catalog estimates as the
// project dashboards without rewriting that source data. A compact lifetime
// aggregate calibrates pricing consistently across callers; requested rows
// stay attached so the transcript can put each estimate on its own footer.
...
// A result event and its ledger row are both written at the end of the same
// turn. Pair them chronologically so old Codex transcripts gain the estimate
// without mutating the stored event. Results with no recorded usage cannot
// have produced a ledger row and must not consume one from a later turn.
...
export async function recordTurnUsage(job, turn, provider, model) {
  ...
  if (!row) return;
  // Usually this settles before the result footer is published, so the live
  // projection can read it. A stalled pool must not hold the completed turn,
  // its queued prompts and its status hostage, though; after one second the
  // write remains best-effort in the background as it was before live costs.
  return new Promise((resolve) => {
    ...
    saveTurnUsage(row)
      .then(() => {
        if (row.costUsd > 0 && row.inputTokens > 0) {
          calibrationRevision++;
          // The aggregate query may have observed this committed row before
          // the INSERT promise settled. Invalidate its snapshot instead of
          // adding the row again; the next projection reloads it exactly once.
          calibrationRows = null;
        }
      })
