...
// say). For a session they are interchangeable: same binary, same models,
// same endpoint, only the quota differs. Those rows form one group, which is
...
export function providerGroupKey(p) {
  if (p.apiKey) return `key:${p.id}`;
  return `${p.binary}|${p.baseUrl || ''}`;
}
...
// The rows interchangeable with this one, itself included, in picker order.
export function providerGroup(p) {
  const key = providerGroupKey(p);
  return cache.filter((row) => providerGroupKey(row) === key).sort(byOrder);
}

// Every group, in the order of its first member: the picker's list. A lone
// row keeps its own label; a group of logins is named after the service,
// since "Claude 1 / 2 / 3" collapsed into one entry is just "Claude".
export function providerGroups() {
  const seen = new Set();
  const groups = [];
  for (const p of [...cache].sort(byOrder)) {
    const key = providerGroupKey(p);
    if (seen.has(key)) continue;
    seen.add(key);
    const members = providerGroup(p);
    const label = members.length === 1 || p.baseUrl ? p.label : getBinary(p.binary).label;
