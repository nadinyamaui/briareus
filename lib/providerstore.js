...
export function providerEfforts(p, model = '') {
  ...
  // an explicit entry effort list to every model. Without one, keep `max`
  // opt-in because an arbitrary endpoint may not support it. Login-backed
  // entries instead use Codex's own per-model metadata, which can differ
  // within one provider.
