...
export function providerEfforts(p, model = '') {
  ...
  // Custom Codex endpoints get a generated catalog that deliberately applies
  // the entry's effort list to every model. Login-backed entries instead use
  // Codex's own per-model metadata, which can differ within one provider.
  if (!model || p.binary !== 'codex' || p.baseUrl || p.apiKey || !binary.effortsForModel) return efforts;
  const supported = new Set(binary.effortsForModel(model, p));
  const narrowed = efforts.filter((effort) => supported.has(effort));
  // An older row may override efforts with values this model cannot run. In
  // that case the model catalog is a safer fallback than an empty picker and
  // an `undefined` effort passed to Codex.
...
export function providerDefaultEffort(p, cfg, model = '') {
