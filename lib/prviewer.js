...
export async function pullRequestView(project, number, options = {}) {
  ...
  async function read(path) {
    const res = await githubRest(cfg, 'GET', `${root}${path}`);
    if (!res.ok) throw new Error(`GitHub answered ${res.status} reading the pull request ${section}`);
    return res.json();
  }
    ...
    for (const r of result.value.rows) {
      checks.push(
        index === 0
          ...
          : {
              name: r.context,
              status: r.state === 'pending' ? 'in_progress' : 'completed',
              conclusion: r.state === 'pending' ? null : r.state === 'success' ? 'success' : 'failure',
              url: r.target_url,
              app: 'Commit status',
