...
const CODEX_MODEL_FALLBACK = ['gpt-6-astra', 'gpt-6-sol', 'gpt-6-luna', 'gpt-5.5', 'gpt-5.4', 'gpt-5.4-mini'];

const CODEX_EFFORTS = ['low', 'medium', 'high', 'xhigh', 'max'];
const CODEX_FALLBACK_MAX_MODELS = new Set(['gpt-6-astra', 'gpt-6-sol', 'gpt-6-luna']);
...
export function codexWideVariants(slugs, provider = null, home = os.homedir()) {
  // Login-backed pickers intentionally omit GPT-5.6 even when a refreshed or
  // manually curated catalog still names it. Custom endpoints do not call
  // this helper, so their operator-defined model lists remain untouched.
  const hadModels = slugs.length > 0;
  slugs = slugs.filter((slug) => !GPT_56_MODEL.test(slug));
  // An older curated row may contain only GPT-5.6 models. Keep that provider
  // launchable with the baked catalog instead of handing Codex an empty -m.
...
// fallback. Hidden, review-only, watermark and GPT-5.6 variants stay out of
// the picker.
...
// A Codex login's model cache is also the authority for which reasoning
// levels a model accepts. Keep the provider-wide list as the set Briareus can
// offer, then narrow it per pick so, for example, GPT-5.5 cannot be launched
// with the GPT-6-only `max` level. When there is no cache yet, the baked list
// stays conservative except for the model families known to support max.
...
const claudeBinary = {
  ...
  buildArgs: ({ model, effort, resume, sessionId, sysPromptFile, mcpConfigFile }) => ({
    args: [
      ...
      '--output-format',
      'stream-json',
      // Each message the CLI takes off stdin is echoed back, which is how
      // runDevTurn tells a message it has read from one still waiting.
    ...
    // stdin stays open after the prompt: a turn that leaves sub-agents or a
    // Monitor working has already answered, and the user's next message goes
    // straight into the same process (see runDevTurn).
...
export function ensureCodexHome(provider, turnModel = '') {
  ...
  const model = picked || splitCodexModel(provider.defaultModel).slug || models[0] || 'gpt-6-sol';
  const lines = [
...
// The background work a claude process waits for before it exits: sub-agents
// and Monitors. A plain `run_in_background` Bash is left out on purpose: print
// mode kills those a few seconds after the last answer, and a dev server
// started that way must not hold the turn open for good.
...
function claudeParser(turn) {
  ...
  // A keep-alive call is only kept until the CLI says whether it went into
  // the background: one live process can make thousands over its life.
  const calls = new Map(); // tool_use id -> what a keep-alive call is doing
  const background = new Map(); // task id -> the same, while it works
  // Every answer of a live process opens with an init. The answer to a
  // message has that message's echo before its first word; a wake-up (a
  // finished task, a Monitor event) has none. Neither has a local command
  // (/context), whose words come from the CLI itself, not a model.
  // Sub-agents stream their own messages in between, so only the main
  // thread's count as the answer's first word.
  let started = false;
  let answer = null; // { echoed, spoke, local } for the answer the latest init opened
  // One process can answer several times (the user's message, a finished
  // sub-agent waking it up). Each result reports its own tokens and time,
  // but a running total of the cost. The turn keeps the process's totals for
  // the accounting at close; each answer's footer shows that answer's own.
  ...
  return {
    feed(msg) {
      ...
      if (msg.type === 'system' && msg.task_id) {
        ...
        // Only a task the CLI says it backgrounded: a foreground Agent call
        // is over when its tool result is, and counting one would hold stdin
        // open after the last answer.
        ...
        // A task the agent stopped itself (TaskStop) wakes nobody: the
        // 2.1.281 CLI reports it "stopped" and starts no answer for it. This
        // comes after the drop that took it off the list.
      ...
      // The echo of a message written to stdin: the CLI has read it, either
      // into the answer it is working on or as the start of the next one.
      // Its text says which messages: several read at once share one echo,
      // and whether it opens an answer of its own or joins one under way.
      // A task's notification that lands while a tool runs is read into the
      // answer under way and echoed the same way. No wake-up follows for it,
      // and it is no message's echo.
      if (msg.type === 'user' && msg.isReplay) {
        const content = msg.message && msg.message.content;
        let text = typeof content === 'string' ? content : null;
        if (text && text.includes('<task-notification>')) {
          const notes = text.match(/<task-notification>[\s\S]*?<\/task-notification>/g) || [];
          for (const note of notes) {
            const id = note.match(/<task-id>([^<]+)<\/task-id>/);
            if (id) out.push({ kind: 'task_settled', id: id[1].trim() });
          }
          text = notes.reduce((rest, note) => rest.replace(note, ''), text).trim();
          if (!text) return out;
        }
      ...
      if (msg.type === 'system' && msg.subtype === 'init') {
        ...
        if (msg.total_cost_usd != null) turn.costUsd = msg.total_cost_usd;
        const part = newTurn();
  ...
  try {
    ...
    if (!res.ok)
      return {
        ...
        error:
          res.status === 429
            ? 'Claude rate limited the usage check. Please try again later.'
            : `Claude usage check failed (HTTP ${res.status}).`,
      };
