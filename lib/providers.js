  ...
  return {
    feed(msg) {
      ...
      if (msg.type === 'system' && msg.task_id) {
        ...
        if (msg.subtype === 'task_started' && calls.has(msg.tool_use_id) && msg.is_backgrounded === true) {
          background.set(msg.task_id, calls.get(msg.tool_use_id));
      ...
      if (msg.type === 'system' && msg.subtype === 'init') {
        ...
        // A live process sends one again when a finished background task
        // wakes it for an answer of its own, which has no echo: this is the
        // only sign that answer has started.
        out.push({ kind: 'init' });
