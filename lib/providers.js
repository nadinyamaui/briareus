...
function claudeParser(turn) {
  ...
  const backgroundEvent = () => ({ kind: 'background', tasks: [...background.values()] });
  return {
    feed(msg) {
      const out = [];
      if (msg.type === 'system' && msg.task_id) {
        ...
        if (background.size !== had) out.push(backgroundEvent());
        return out;
      }
      ...
      if (msg.type === 'system' && msg.subtype === 'background_tasks_changed' && Array.isArray(msg.tasks)) {
        ...
        const had = background.size;
        for (const id of background.keys()) if (!ids.has(id)) background.delete(id);
        if (background.size !== had) out.push(backgroundEvent());
        return out;
      }
      ...
      if (msg.type === 'user' && msg.isReplay) {
        if (answer && !answer.spoke) answer.echoed = true;
        const content = msg.message && msg.message.content;
        return [{ kind: 'ack', text: typeof content === 'string' ? content : null }];
      }
