...
export function parseRunProfiles(text) {
  ...
  lines.forEach((raw, i) => {
    ...
    // Any other character ensureDatabase refuses, rendered with stand-ins for
    // the placeholders a database name can be built from (a {dir} or {host}
    // is left as typed, braces and all, since neither makes an identifier).
    if (key === 'DB_DATABASE' && value) {
      const sample = render(value, { database: 'db', profile: current.name, port: '8100' });
