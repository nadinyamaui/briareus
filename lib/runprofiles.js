...
export function parseRunProfiles(text) {
  ...
  lines.forEach((raw, i) => {
    ...
    for (const [, tenant] of line.matchAll(/\{host:([\w-]+)\}/g))
      hostTokens.push({ profile: current, tenant, where });
    if (section === 'before') {
    ...
    // Quotes are how a .env writes a value with spaces; the shell never sees
    // this one, so they are taken off rather than passed through.
...
export function render(template, vars) {
  return String(template ?? '').replace(/\{(\w+(?::[\w-]+)?)\}/g, (whole, key) =>
