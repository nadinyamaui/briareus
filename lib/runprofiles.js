...
export function parseRunProfiles(text) {
  ...
  lines.forEach((raw, i) => {
    ...
    // else), rendered with stand-ins for the placeholders a database name can
    // be built from. The rest ({port}, {dir}, {host}) are left as typed, braces
    // and all, so they are refused: {port} is not the same on every path that
    // renders the name (the claim-time drop has none to give it).
...
// was saved, but a row saved before a rule was added, or written some other
// way, may no longer read: treated as no profiles at all rather than taking
// every caller down with it, and logged once per text. ▶ Run asks
// runProfilesError instead of quietly serving the plain run commands.
