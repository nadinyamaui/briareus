...
export const WORKSPACE_PRUNE_INTERVAL_MS = 24 * 60 * 60 * 1000;
...
// Remove whole clone slots that no open session owns. This runs synchronously
// on purpose: after claimant() says a slot is idle, the event loop must not get
// a chance to hand that same directory to a new session before it is removed.
...
// Run once at boot so frequent restarts cannot postpone cleanup forever, then
// once per day for as long as the dashboard process remains alive.
export function startWorkspacePruner(log = console.log, intervalMs = WORKSPACE_PRUNE_INTERVAL_MS) {
  const run = () => {
    ...
    if (result.removed.length) log(`Workspace cleanup removed ${result.removed.length} idle slot(s).`);
    for (const failure of result.errors)
      log(`Workspace cleanup could not remove ${failure.slot}: ${failure.error}`);
    return result;
  };
  run();
