...
function previewTunnelConfig() {
  ...
  // (*.zone) will not cover it; that part is Cloudflare's to reject.
  const hostname = reqCf('PREVIEW_HOSTNAME').toLowerCase();
  if (hostname && !/^[a-z0-9.-]*\{port\}[a-z0-9.-]*$/.test(hostname)) {
    missing.push(`PREVIEW_HOSTNAME (a hostname with {port} in it: ${hostname})`);
  }
