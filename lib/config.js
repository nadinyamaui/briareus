...
// Also what ▶ Run reads a local checkout's .env with, so it takes the forms a
// Laravel .env is written in: `export KEY=` and spaces around `=`. A trailing
// ` # comment` after the value is only cut with `inlineComments`: this app's
// own .env has always been read with ` #` as part of an unquoted value (a
// password, a token), and cutting it there would change what an existing
// install logs in with.
/**
 * @param {string} text
...
function findClaudeBin(envOverride) {
  ...
  // The bundled copy only moves when somebody runs `npm ci` after a deploy;
  // the machine's own install moves with `claude update`. Left in PATH order
  // the bundled one shadows the install the operator just updated, and a new
  // model then fails every turn with "Claude Code x.y.z does not support this
  // model" while `claude --version` in a shell says otherwise. So it is the
  // last resort, behind every install the operator maintains by hand.
  const bundled = [];
...
// An optional integration's settings, all or nothing: null when none of its
// keys says anything, and otherwise a reader that records each one left
// empty. These may come from the server's process environment as well as
// .env, and the environment wins whenever it says something (see R2 below).
/** @param {string[]} keys */
function optionalGroup(keys) {
...
// before it reaches any child (childEnv in lib/childenv.js), so the
...
// The optional Cloudflare tunnel ▶ Run publishes app ports through, for a
// dashboard reached from outside this machine: there, the 127.0.0.1 URL a
// served app answers on is the viewer's own computer. Same all-or-nothing
// rule as R2 (a token with no Access emails would publish previews nobody is
// guarding), and the same home: the API token can rewrite DNS, Access and
// the tunnel, so the process environment, stripped of CLOUDFLARE_* before any
// job child sees it, is where it belongs rather than the agent-readable .env.
function previewTunnelConfig() {
  ...
  // One hostname per port, so the template must carry the port, and must stay
  // one label under the zone's apex or Cloudflare's universal certificate
  // (*.zone) will not cover it; that part is Cloudflare's to reject. A run
  // profile's tenants get a hostname each on the same port; `{tenant}` says
  // where the key goes, and it has to share the port's label for the same
  // certificate reason.
...
// The optional transcription behind the composer's 🎤: a voice note is
// recorded in the browser and sent here, and the server hands it to OpenAI's
// transcription API, so the key never reaches the page. Off when neither key
// is set; once one is, both are required. The model is named rather than
// assumed for the same reason CLAUDE_MODEL is: a hardcoded one keeps being
// sent after it is retired. Like R2_*, both may come from the process
// environment, stripped of OPENAI_TRANSCRIBE_* before any job child sees it:
// the key bills an OpenAI account, and a session has no use for it.
