...
function previewTunnelConfig() {
  ...
  if (
    hostname &&
    (!/^[a-z0-9.-]*\{port\}[a-z0-9.-]*$/.test(hostname.replace('{tenant}', '')) ||
      hostname.split('{tenant}').length > 2 ||
      (hostname.includes('{tenant}') && !firstLabel.includes('{tenant}')))
  ) {
    missing.push(
      `PREVIEW_HOSTNAME (a hostname with {port} in it, and {tenant} if any in its first label: ${hostname})`,
    );
  }
