...
// Session lists wait for this refresh on a cold cache, so a third party that
// accepts the connection but stalls must not hold the dashboard open.
...
export function priceFor(catalog, binaryId, model) {
  // Codex's picker stores an optional context-window choice in the label
  // (`gpt-6-astra (872k)`), while the pricing catalog knows the underlying
  // model slug only. It is still the same model and rate card.
...
export function withEstimates(rows, catalog, calibrationRows = rows) {
  ...
  const cacheShare = cacheShareOf(calibrationRows, priceOf);
  return rows.map((r) => {
