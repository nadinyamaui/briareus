...
function newReviewLoop() {
  return {
    ...
    // (loopSessionRuntime) for a fix session and the QA run; a review reads it
    // as "the reviewer this project was set up with, and the session only when
    // there is none, or none that can run" (loopReviewChoice). A retry that
    // named only a model or an effort has the rest filled in from that
    // reviewer, so this can hold the reviewer itself: loopReviewChoice treats
    // that as riding the project rather than as an override over it.
    runtime: null,
    ...
    // repair. Nothing clears it while the loop lives: repairing the reviewer
    // and turning the 🔁 chip off and on builds a new loop (newReviewLoop),
    // which asks the project again, and both give-up messages say so.
...
// provider filled in from the reviewer (retryLoopRound), so an override can be
// the project's reviewer without anybody having chosen it, and reading that as
// "somebody moved this loop off the project" would silently switch off both
// safety nets around a failing reviewer.
function loopReviewChoice(job) {
  ...
  const override = loop && loop.runtime ? resolveRuntime(loop.runtime, cfg) : null;
