...
function jobEnv(overrides = {}, job = null) {
  ...
  // The Cloudflare token that publishes previews can rewrite DNS and Access,
  // so it is stripped for the same reason.
  for (const key of Object.keys(env))
    if (key.startsWith('R2_') || key.startsWith('CLOUDFLARE_')) delete env[key];
