...
export function createDevSession({
  ...
  const job = {
    ...
    chats: {},
    // Which account each step's provider group was balanced onto for this
    // session, keyed by group (see stepProvider). Decided on the step's first
    // turn and kept, so the step's conversation stays on one login.
    stepProviders: {},
    turns: 0,
    costUsd: null,
...
export function stepProvider(job, provider) {
  ...
  if (own && providerGroupKey(own) === key) return own;
  if (!job.stepProviders) job.stepProviders = {};
  const pinned = getProvider(job.stepProviders[key]);
  if (pinned && providerGroupKey(pinned) === key) return pinned;
