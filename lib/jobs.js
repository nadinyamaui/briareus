...
export function setReviewLoop(id, on) {
  ...
  if (on) {
    ...
    if (running) {
      ...
      // The head that review is reading, recorded the way maybeStartLoopReview
      // records it when it starts a round itself: the hold (holdForTriage)
      // carries it as the sha the round was reviewed at, and the commit gate
      // compares against it. Left unset, the round could never be flagged
      // stale and a nothing-to-fix send would re-review this same commit.
      job.reviewLoop.lastSha = loopHeadSha(job);
    }
    pushEvent(job, 'info', {
      text: running
...
// found is on the pull request, and the verdicts it waited for are moot.
...
function markHeldRoundStale(job) {
  ...
  if (!held || held.stale || !held.sha) return;
  const sha = loopHeadSha(job);
  if (!sha || sha === held.sha) return;
  held.stale = true;
  pushEvent(job, 'info', {
...
// starts until they do (triageLoopFindings). The round cap still stands ahead
// of it, on what the loop's own rules would have fixed: a final round that
// leaves something the loop itself would implement can start no fix session,
// so it has nothing to triage for and stalls as it always did. One whose
// findings the rules would all have parked is held like any other, so the
// person can rule it through and a nothing-to-fix send converges the loop
// the way the cap never stood in the way of on rounds that fixed nothing.
async function holdForTriage(parent, prNumber, findings) {
  ...
  if (cfg.reviewLoop.maxRounds && loop.rounds >= cfg.reviewLoop.maxRounds && sorted.kept.length) {
    loop.stalled = true;
    pushEvent(parent, 'info', {
      text: `Review loop: round ${loop.rounds} is as far as this loop goes (REVIEW_LOOP_MAX_ROUNDS=${cfg.reviewLoop.maxRounds}), so its ${open.length} finding(s) are listed on PR #${prNumber} rather than implemented. Read them and decide: ⚙ Implement feedback does the round, and the 🔁 chip restarts the loop from scratch.`,
    });
    save(parent);
    bus.emit('job', publicJob(parent));
    notifyParentStalled(
...
export async function triageLoopFindings(id, { verdicts, note, by = 'The orchestrator' } = {}) {
  ...
  pushEvent(job, 'info', {
    text:
      `Review loop: ${by.toLowerCase()} triaged round ${held.round}: ${kept.length} finding(s) to fix, ${waived} left out` +
      (waived
        ? ` (${all
  ...
  if (!kept.length) {
    ...
    // The commit gate is re-run the way any settle runs it.
    const sha = loopHeadSha(job);
    if (sha && sha !== loop.lastSha) {
      pushEvent(job, 'info', {
      ...
      return { fixing: false, converged: false };
    }
    loop.done = true;
    pushEvent(job, 'info', {
...
async function syncDevPrNow(job, number = null, { spotted = false, strict = false } = {}) {
  ...
  try {
    ...
    } else if (before && before.headSha !== status.headSha && job.reviewLoop && job.reviewLoop.triage) {
      // The stale check reads the clone's own remote-tracking ref
      // (loopHeadSha), which a push made anywhere but this clone (by hand,
      // from another checkout) never moves; nothing else fetches while a
      // round is held. So teach the clone about the head the sync just saw
      // first, or the probe answers the reviewed sha and the card stays
      // fresh over commits no round has read.
      await fetchLoopBranch(job);
      markHeldRoundStale(job);
    }
    bus.emit('job', publicJob(job));
    save(job);
