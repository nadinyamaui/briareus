...
// A hand-started ⌕ Code review is deliberately one-shot, and what it found is
// for the pull request's author to fix, not for whoever ran the review. So the
// card it leaves is a queue card and not a gate: it says the review has not
// been read yet, and what it offers acts on the review rather than ruling on
// it — a reply on a finding's thread (replyToReviewFinding), deleting one from
// the review (deleteReviewFinding), and Complete (completeStandaloneReview).
...
export async function holdStandaloneReviewFindings(job) {
  ...
  pushEvent(job, 'info', {
    text: `Code review: ${open.length} finding(s) are waiting in ⚑ Findings. Read them there and complete the review; they stay on the pull request for its author to fix.`,
  });
  save(job);
  bus.emit('job', publicJob(job));
  return true;
...
// Complete on a one-shot review's queue card. A review started by hand is a
// review of somebody else's pull request, and its author is who answers it,
// not the person who ran it. So there is nothing to rule on here: what the
...
export async function saveReviewFindingsDrafts(id, { verdicts, note, by = 'the user' } = {}) {
  const job = jobs.get(id);
  if (!job || job.kind !== 'devchat') throw new Error('Session not found');
  // Loop rounds only: a hand-started review is not triaged here (its card is
  // Complete and nothing else), so it has no verdicts and no comments to keep.
  const held = job.reviewLoop && job.reviewLoop.triage;
...
// The developer UI accepts either source of queue cards. The agent endpoint
// remains loop-only, so an orchestrator cannot accidentally rule on a review
// the user started by hand.
export async function triageReviewFindings(id, opts = {}) {
  const job = jobs.get(id);
  if (job && job.reviewTriage) return completeStandaloneReview(id, opts);
