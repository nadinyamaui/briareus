...
async function readPrDetails(cfg, repo, number, status) {
  ...
  try {
    ...
    const data = await githubGraphql(cfg, PR_DETAILS_QUERY, { owner, name, number });
    node = data?.repository?.pullRequest;
  } catch (e) {
    // A query whose parts failed still answers the parts that resolved.
    node = e?.data?.repository?.pullRequest;
  }
  if (!node) return;
  const lower = (v) => String(v || '').toLowerCase();
  // The review verdicts, one per reviewer. Chronological fold: a real verdict
  ...
  if (node.reviews?.nodes) {
    ...
    for (const r of node.reviews.nodes) {
      ...
      if (!login || state === 'pending') continue;
      ...
      if (
        ...
        continue;
      latest.set(login, { user: login, state: verdict ? state : 'commented', url: r.url || null });
    }
  ...
  // The head commit's check runs. A commit with no rollup has no checks at
  // all; that is an answer too (total 0), not a failed read.
  const headNode = node.head?.nodes?.[0]?.commit;
  if (headNode && 'statusCheckRollup' in headNode) {
    const runs = (headNode.statusCheckRollup?.contexts?.nodes || [])
      .filter((c) => c.__typename === 'CheckRun')
      .map((r) => ({
        name: r.name,
        status: lower(r.status),
        conclusion: r.conclusion ? lower(r.conclusion) : null,
        url: r.detailsUrl || null,
      }));
    status.checks = {
      total: runs.length,
...
async function syncDevPrNow(job, number = null, { spotted = false, strict = false } = {}) {
  ...
  try {
    ...
    // there is something new to read: a pull request GitHub answered 304 for
    // has had no push, review or comment since the last sync, so the mirrored
    // lists still stand. A CI run can finish without touching the pull request
    // itself, so pending checks keep the query going until they settle.
    const before = job.prStatus;
    const unchanged =
      !!pres.notModified && !!before && before.number === pr.number && before.headSha === status.headSha;
    const checksSettled = !!(before && before.checks) && before.checks.pending === 0;
    if (!unchanged || !checksSettled) await readPrDetails(cfg, job.repo, number, status);
    job.prStatus = status;
...
// The repositories whose GitHub webhook points at this install (lib/webhooks.js
// says so after installing it at boot). A session there hears about every
// push, review, comment and CI verdict as it happens, so the timer only has
// to cover a lost delivery.
const hookedRepos = new Set(); // lowercased owner/name
const HOOKED_SYNC_MS = 15 * 60_000;

export function markRepoWebhook(repo, live) {
  const key = String(repo || '').toLowerCase();
  if (!key) return;
  if (live) hookedRepos.add(key);
  else hookedRepos.delete(key);
}

export function repoHasWebhook(repo) {
  return hookedRepos.has(String(repo || '').toLowerCase());
}
...
function syncDevPrs() {
  for (const job of jobs.values()) {
    ...
    const cadence = ACTIVE.includes(job.status) ? 0 : repoHasWebhook(job.repo) ? HOOKED_SYNC_MS : 55_000;
