...
function reconcileRestartedLoopJobs(restored) {
  for (const parent of restored) {
    ...
    if (qa?.qaParentId === parent.id) {
      ...
      pushEvent(parent, 'info', {
        text: 'QA loop: the QA session was interrupted by a server restart. QA is retried when this session next settles.',
      });
      save(parent);
    }
  }
...
// them: three independent complete proposals. Each must be given a
...
export const ZEUS_PROPOSAL_ROLES = ['product', 'architecture', 'qa'];
// Keep validator readable for existing sessions; new fusion runs use Zeus to summarize.
export const ZEUS_ROLES = [...ZEUS_PROPOSAL_ROLES, 'validator'];
...
export function createDevSession({
  ...
  if (zeus && (!roles || ZEUS_PROPOSAL_ROLES.some((role) => !roles[role]))) {
    throw new Error('Choose a model for all three proposal slots before starting Zeus');
  }
...
export function sendDevMessage(id, text, attachments, zeusRoles) {
  ...
  if (zeusRoles != null) {
    ...
    if (!roles || ZEUS_PROPOSAL_ROLES.some((role) => !roles[role])) {
      throw new Error('Choose a model for all three proposal slots');
    }
    job.zeusRoles = roles;
    save(job);
    ...
    prompt += `\n\n<workspace-context>\nThe user has now chosen all three proposal models. \
These choices supersede any earlier missing or partial picks. ${zeusRolesAdvice(job)}
Use role product for Model 1, architecture for Model 2 and qa for Model 3. \
The proposal slots are complete independent proposals, not specialties; send the same complete proposal prompt to all three.
ZEUS itself consolidates all three complete outputs; do not split the task into phases or spawn a separate validator.
...
async function maybeStartLoopReview(job, { fresh = false } = {}) {
  ...
  if (loop.pendingResult) {
    ...
    // is retried here, and the loop moves on (converges, triages, fixes)
  ...
  if (loop.triage) {
    // Waiting on the orchestrator's verdicts. One that is gone (closed, or
    // deleted with the worker left open) will never give them, so the round
    // goes the way a loop with no orchestrator runs it, rather than holding
    // the worker's loop for good on a decision nobody is going to make.
    if (loopTriager(job)) return;
    const held = loop.triage;
    loop.triage = null;
    pushEvent(job, 'info', {
      text: `Review loop: the orchestrator that was to triage round ${held.round}'s findings is gone, so they are implemented as the loop would on its own.`,
    });
    save(job);
    bus.emit('job', publicJob(job));
...
async function resolveLoopRound(parent) {
  ...
  // still goes on to triage or a fix session below: both start a session of
  // their own rather than resuming the parent's own turn (startLoopFixSession,
  // triage_findings), so neither needs the parent itself to be live.
  loop.pendingResult = null;
  ...
  // A worker's round is its orchestrator's to judge before anything is
  // implemented: it has the task's whole context, which the review did not,
  // and a finding it knows to be beside the point costs nothing when it never
  // reaches a fix session. A loop with nobody to ask runs the round itself.
  if (loopTriager(parent)) await holdForTriage(parent, prNumber, findings);
  else await implementRound(parent, prNumber, findings);
}
...
// The orchestrator a worker's round is triaged by: its parent, while that
// parent is not closed. A closed orchestrator takes no updates
// (queueWorkerNotice drops them), so a round held for it would hold forever.
function loopTriager(job) {
  if (!job.parentId) return null;
  const parent = jobs.get(job.parentId);
  if (!parent || parent.kind !== 'devchat' || !parent.orchestrator || parent.status === 'closed') return null;
  return parent;
}

// Hold a round for its orchestrator. The findings are sorted the way the
// loop would sort them on its own, and the split goes along as advice rather
// than as verdicts: what the rules would have parked is marked so, and the
// orchestrator decides. Nothing is recorded and no fix session starts until
// it does (triageLoopFindings). The round cap still stands ahead of it: a
// round that can start no fix session has nothing to triage for, and stalls
// as it always did, handing the orchestrator the findings to send by hand.
async function holdForTriage(parent, prNumber, findings) {
  ...
  try {
    ...
    // their account: the orchestrator sees the whole round and rules again.
    pushEvent(parent, 'info', {
  ...
  if (cfg.reviewLoop.maxRounds && loop.rounds >= cfg.reviewLoop.maxRounds) {
    loop.stalled = true;
    pushEvent(parent, 'info', {
      text: `Review loop: round ${loop.rounds} is as far as this loop goes (REVIEW_LOOP_MAX_ROUNDS=${cfg.reviewLoop.maxRounds}), so its ${open.length} finding(s) stay on PR #${prNumber} for the orchestrator to judge rather than being implemented.`,
    });
    save(parent);
    bus.emit('job', publicJob(parent));
    notifyParentStalled(
  ...
  loop.triage = { prNumber, round: loop.rounds, findings: open };
  pushEvent(parent, 'info', {
    text: `Review loop: review round ${loop.rounds} left ${open.length} finding(s), waiting for the orchestrator to triage them. Only what it marks fix goes to the fix session.`,
  });
  save(parent);
  bus.emit('job', publicJob(parent));
...
// One finding as the orchestrator reads it: key first, since the key is what
// its verdict has to name, then what the review said and what the rules
// would have done with it.
function fmtTriageFinding(f) {
  const loc = f.file ? ` (${f.file}${f.line ? `:${f.line}` : ''})` : '';
  const advice = f.parked ? ` — the loop would have parked it: ${PARK_REASONS[f.parked] || f.parked}` : '';
  return `[${f.key || findingKey(f.title)}] ${String(f.severity || 'medium').toUpperCase()}: ${f.title}${loc}${advice}`;
}

function notifyParentTriage(worker, prNumber, round, findings) {
  queueWorkerNotice(worker, 'triage', {
    text:
      `Worker ${worker.id} (${worker.title || 'untitled'}): review round ${round} of PR #${prNumber} left ${findings.length} finding(s) for you to triage before anything is fixed:\n` +
      findings.map((f) => `- ${fmtTriageFinding(f)}`).join('\n') +
      `\nRule on every one with triage_findings: fix (it goes to the fix session), dismissed (it does not apply; say why) or optional (real but not worth a round now). The loop holds until you do; read the pull request first if a finding needs the diff to judge.`,
  });
}

// The orchestrator's verdicts on the round its worker's loop is holding.
...
function notifyQaLoopFailed(qa) {
  ...
  if (!DEV_OPEN.includes(parent.status) && parent.status !== 'interrupted') return;
  pushEvent(parent, 'info', {
    text: `QA loop: the QA session failed (${qa.error || 'no error recorded'}). QA is retried when this session next settles.`,
  });
  save(parent);
}
...
async function onQaLoopClosed(qa) {
  ...
  if (!qa.qaLoopDone) {
    pushEvent(parent, 'info', {
      text: 'QA loop: the QA session was stopped before it finished. QA is retried when this session next settles.',
    });
    save(parent);
    return;
  }
...
function notifyParentChecks(worker, status) {
  ...
  const wait = !loop
    ? null
    : loop.stalled
      ? 'its review loop is stalled and left findings for you to judge'
      : !loop.done
...
// The findings a stalled loop left are the orchestrator's to judge: some go
// back to the worker, some are waived. Titles only: the pull request carries
// the detail, and a whole finding list would crowd the turn.
function notifyParentStalled(worker, prNumber, why, left) {
  const titles = left.map((f) => f.title).join('; ');
  notifyParentLoop(
    worker,
    `the review loop stalled on PR #${prNumber}: ${why}, leaving ${left.length} finding(s) listed on the pull request: ${titles}. Judge each: send the ones that apply to the worker with send_to_worker (its push resumes the loop); a finding that does not apply is yours to waive, saying why on the pull request.`,
  );
}
...
async function prepareDevWorkspace(job) {
  ...
  await runCmd(job, 'git', ['-C', dir, 'fetch', '--progress', '--no-tags', 'origin', ...refspecs]);
...
environment variables:
    ...
    curl -sS -X POST "$REVIEWER_MEMORY_URL/api/agent/sessions/<id>/triage" -H "Authorization: Bearer $REVIEWER_MEMORY_TOKEN" \
      -H 'Content-Type: application/json' -d '{"verdicts":[{"key":"…","decision":"fix|dismissed|optional","reason":"…"}],"note":"…"}'
...
request is code-reviewed, and the findings come to you to triage before anything is fixed (below); what you \
mark fix goes to a fix session of its own, round after round until a review finds nothing. Those reviews and \
...
still reviewing, waiting on your triage, fixing or running QA is not done, so do not close it or report the \
task finished until the loops converge, stall or fail.

# Review triage

Every review round of a worker's loop stops at you: the findings arrive as an update, each with its key, \
severity, file and, where the loop's own rules would have set it aside (below the round's severity floor, or \
on a file the pull request does not touch), a note saying so. You know the task; the review did not. Rule on \
every finding with triage_findings: fix (it goes to the fix session), dismissed (it does not apply — a \
reviewer reading without the task's context, a deliberate choice, out of scope; say why in reason) or \
optional (real, not worth a round now). The dismissals and their reasons are recorded on the pull request, \
and a dismissed finding is never offered again, so be sure. A note rides into the fix session as extra \
guidance. The loop holds until you rule: a round you leave untriaged is a worker stuck. If the update was \
lost, list_workers shows the round waiting and read_worker shows its findings. Keeping everything is a \
verdict too; do not dismiss to save a round on code that is meant to land.
...
worker, triage a review round's findings with triage_findings, judge the findings a stalled loop left. \
...
you from a web UI and reads your replies there. They give you a brief; three models independently solve the same complete task, and you consolidate their outputs and \
...
Your session model is the summarizer. All three selected models do the same complete task independently, \
then you combine their outputs into one final epic. There are no specialist assignments, phased analyst \
workflow or separate validator. Before spawning, ensure the user has chosen all three models; if choices \
...
Start three analysts with spawn_worker, passing exactly the SAME prompt string to all three. Do not divide \
the task, change emphasis, assign phases or share their outputs with one another. Use role: product for \
Model 1, architecture for Model 2 and qa for Model 3; these legacy identifiers select runtimes only, not \
specialties. Use neutral titles Model 1, Model 2 and Model 3. Analysts are read-only and cannot publish.

Wait for all three outputs. Read each proposal with read_worker using full_text: true and a small tail \
