...
export function publicJob(job, ledgerUsage = null) {
  ...
  const { events, seq, proc, timeout, turnCanceled, turnTimedOut, serveProc, ...meta } = job;
...
function startDevSession(job, prompt) {
  (async () => {
    try {
      ...
      // working fine was simply given a job too big for the limit.
      const fromProvider = job.status === 'running' && !job.turnTimedOut;
      if (job.status !== 'closed') {
...
// The provider the project's ⌕ Code review names right now, or null when it
// names none, or one that no longer resolves. What a retry reads to tell "run
// on the project's reviewer again" from "move this loop somewhere else".
...
function giveReviewerUp(job) {
  ...
  for (const field of ['runtime', 'reviewRuntime']) {
    ...
    if (resolved && resolved.provider.id === reviewer) loop[field] = null;
  }
...
async function maybeStartLoopReview(job, { fresh = false } = {}) {
  ...
  try {
    ...
    try {
      ...
      giveReviewerUp(job);
      pushEvent(job, 'info', {
        text: `Review loop: the reviewer this project was set up with cannot run (${e.message}), so this round and the ones after it run on ${job.provider} instead. Repair it in Settings and turn the 🔁 chip off and on to ask the project again${qaRearmNote(job)}.`,
      });
      review = fallback;
    }
...
export async function retryLoopRound(id, { providerId, model, effort } = {}) {
  ...
  if (providerId != null || model != null || effort != null) {
    ...
    if (providerId != null) {
      ...
      // repaired — the account rolled over, the CLI is installed now — so take
      // the flag off with it. Left on, loopReviewChoice would drop the
      // override it was just handed and review on the session's provider while
      // the fix sessions moved onto the provider the review refused.
      if (loop.reviewerFailed && resolved.provider.id === projectReviewerProviderId(job))
...
function notifyLoopReviewFailed(review, { fromProvider = false } = {}) {
  ...
  // one of the project's setup steps, a turn the time limit cut off — and the
  // round is retried on the reviewer as it always was.
  const gaveUp = fromProvider && !!parent.reviewLoop.reviewerRound;
  if (gaveUp) giveReviewerUp(parent);
  ...
  pushEvent(parent, 'info', {
    text: `Review loop: the code review failed (${why}), so this round reviewed nothing.${
      gaveUp
        ? ` The reviewer this project was set up with is what failed, so the rounds after this one run on ${parent.provider} instead — repair it in Settings and turn the 🔁 chip off and on to ask the project again${qaRearmNote(parent)}.`
        : ''
  ...
  // one of the project's setup steps or the turn's own time limit ended says
  // nothing about the provider it was riding, and moving off the reviewer this
  // project was set up with for one of those is the very thing the gate above
  // exists to prevent.
  notifyParentLoop(
    parent,
    fromProvider
      ? `the review loop's code review could not run (${why}). This is the provider failing, not a review that found nothing: round ${parent.reviewLoop.rounds} approved nothing. Retry it with retry_review — on a different provider_id (another service, since one provider_id's accounts are already balanced by quota) or model if this one is out of quota — since a worker with nothing left to push will never start the round itself.`
      : `the review loop's code review could not run (${why}). The round was interrupted rather than turned away by the provider — a restarted dashboard, the workspace clone or one of the project's setup steps, a turn the time limit cut off — so round ${parent.reviewLoop.rounds} approved nothing. Retry it as it was with retry_review, naming no provider_id or model: nothing here says the runtime it ran on is the problem, and naming one would move this loop's reviews, its fix sessions and its QA run off it — since a worker with nothing left to push will never start the round itself.`,
  );
}
