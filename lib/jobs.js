...
export function publicJob(job, ledgerUsage = null) {
  ...
  // Answered, with sub-agents or Monitors still working: the panel says so
  // instead of a bare "working…", and the composer's next message goes in now.
  meta.waitingOnBackground = !!(liveInputs.get(job.id)?.idle && job.backgroundTasks?.length);
...
// A claude turn that has answered but is still waiting on its sub-agents or
// Monitors keeps its CLI, and that CLI's stdin, alive. A message sent then
// goes straight in (the agent answers now, with the background work still
// going) instead of waiting in devQueues for everything to finish.
const liveInputs = new Map(); // job id -> { idle, send(prompt) }
...
// Hand a message to the turn that is waiting on background work, the way
// drainDevQueue would have sent it once that turn ended.
function sendToLiveTurn(job, input, entry) {
  if (entry.newFusionBrief) {
    job.zeusProposalPrompt = null;
    save(job);
  }
  pushEvent(job, 'user', { text: entry.shown, ...attachmentMeta(entry.files) });
  input.send(entry.prompt);
  bus.emit('job', publicJob(job));
}
...
${zeusRolesBriefing(job)}\n</workspace-context>`;
  ...
  if (ACTIVE.includes(job.status)) {
    ...
    if (input && input.idle && !devQueues.get(job.id)?.length) {
      sendToLiveTurn(job, input, { prompt, shown, files, newFusionBrief });
    } else {
...
function runDevTurn(job, prompt, { review = false, step = null } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.stdin.on('error', () => {});
    // A stream-json CLI keeps stdin open while its answer leaves background
    // work running; settleInput closes it the moment nothing is left to wait
    // for, which is what lets the process exit.
    let input = null;
    if (build.promptVia === 'stream-json') {
      input = {
        ...
        send(text) {
          input.idle = false;
          child.stdin.write(claudeInputMessage(text));
        },
      };
    ...
    const settleInput = () => {
      if (!input || !input.idle || liveInputs.get(job.id) !== input) return;
      if (!job.backgroundTasks.length || job.turnCanceled || job.status === 'closed') {
        liveInputs.delete(job.id);
        child.stdin.end();
        bus.emit('job', publicJob(job));
        return;
      }
      // Answered, with sub-agents still out: what was typed meanwhile goes
      // in now, one message per answer, as it would have after the turn.
      const queue = devQueues.get(job.id);
      if (queue?.length) {
        const next = queue.shift();
        if (!queue.length) devQueues.delete(job.id);
        sendToLiveTurn(job, input, next);
      } else {
        bus.emit('job', publicJob(job));
      }
    };
        ...
        try {
          for (const e of parser.feed(JSON.parse(line))) {
            ...
            if (e.kind === 'background') {
              job.backgroundTasks = e.tasks;
              save(job);
              continue;
            }
            if (e.kind === 'result' && input) input.idle = true;
            const { kind, ...data } = e;
