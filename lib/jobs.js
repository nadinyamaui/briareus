...
export async function retryLoopRound(id, { providerId, model, effort } = {}) {
  ...
  if (providerId != null || model != null || effort != null) {
    ...
    // loop, and what it settles goes to the fix sessions and the QA run
    // (loopSessionRuntime), so it keeps the model and effort those already
    // have: the reviewer's tier is nobody's answer for code-writing turns.
    const choice = loopReviewChoice(job);
    const base = providerId != null ? loopSessionRuntime(job) : choice.runtime;
...
list_workers says the round failed, and retry_review re-runs it (on a different service's provider_id, or another \
model, when that is the problem), which is the only thing that will, \
