...
// `status` only when the answer carries it, so a query that fails (or a token
// that cannot read one of the fields) keeps whatever the last sync mirrored:
// best-effort, like the rest of the sync. The checks are read off the commit
// the REST answer named as the head, not off the pull request's commit list:
// that list stops at 250 commits, and a push landing between the two reads
// would otherwise pair the new head's checks with the old head's sha.
...
async function readPrDetails(cfg, repo, number, status) {
  ...
  } else if (errorsOn(checksPath).some((er) => PERSISTENT_GRAPHQL_ERRORS.has(er?.type))) {
...
async function syncDevPrNow(job, number = null, { spotted = false, strict = false, details = false } = {}) {
  ...
  try {
    ...
    // (running, or a fresh head whose CI had not registered yet), or the head
    // moved under it, which is a push it saw land. The first sync of a pull
    // request says nothing — a fresh attach and the first tick after a restart
    // both find a finished run that nobody is waiting on any more.
    const rollup = ciChecks(status.checks);
    const settled = rollup && rollup.total > 0 && rollup.pending === 0;
    const watched = before && (before.headSha !== status.headSha || checksAwaited(before));
