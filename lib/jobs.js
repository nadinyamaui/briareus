...
// migrate, a domain registration) expect it to be there. `env` is the profile's
// env as the caller already rendered it (▶ Run's recipe); the callers without
// one get it rendered here.
async function prepareProfileDatabase(job, profile, env = null) {
  if (!profile) return;
  const database = runDatabaseName(job);
  if (!env) {
    const vars = runVars({
...
async function serveInTurn(job, wanted) {
  ...
  if (running && same && job.serveRecipe === recipeKey(devServeRecipe(job, profile), profile)) {
    return servedLinks(job, profile);
  }
  ...
  const recipe = await prepareServe(job, profile, dir);
...
async function prepareServe(job, profile, dir) {
  const recipe = devServeRecipe(job, profile);
  if (recipe.notReady) throw new Error(recipe.notReady);
  serveCalledOff(job, dir);
  await prepareProfileDatabase(job, profile, recipe.env);
  return recipe;
}
...
// different app to the Host-header lookup; otherwise the port's one. `url` is
// the tab ▶ opens: the first of them.
async function servedLinks(job, profile) {
  ...
  const portHost =
    tenants.length && usesPlaceholder(profile, project ? project.runCommands : [], 'host')
      ? servedUrl(job, port)
      : null;
  const links = tenants.length
    ? await Promise.all(tenants.map(async (tenant) => ({ tenant, url: await servedUrl(job, port, tenant) })))
    : [{ tenant: null, url: await servedUrl(job, port) }];
  await portHost;
  // The run may have been stopped while the publish was in flight.
  if (job.serveProc && job.serveProc.exitCode === null) {
    job.serveLinks = links;
    save(job);
    bus.emit('job', publicJob(job));
  }
  return { url: links[0].url, links, profile: profile ? profile.name : null };
}
...
async function startDevServeProc(job, profile, recipe = null, dir = job.workDir) {
  ...
  const served = await published;
  const tenants = served.links.filter((l) => l.tenant);
  if (tenants.length) {
    pushEvent(job, 'info', {
      text: `Tenant links: ${tenants.map((l) => `${l.tenant} ${l.url}`).join(', ')}`,
      links: served.links,
    });
  } else if (served.url !== local) {
    pushEvent(job, 'info', { text: `Published through the tunnel at ${served.url}`, links: served.links });
  }
  return served;
}
