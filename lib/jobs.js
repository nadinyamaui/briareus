...
// A second click reuses the pristine preview already holding that branch, so
// it also reuses the app server and URL. Once somebody chats in the session it
// is no longer pristine and a later board click takes a fresh checkout.
export async function startPullRequestPreview({ provider, model, effort, repo, branch, prNumber, title }) {
  let job = [...jobs.values()].find(
    (candidate) =>
      candidate.kind === 'devchat' &&
      candidate.preview &&
      candidate.repo === repo &&
      candidate.startBranch === branch &&
      candidate.startedOnPr === prNumber &&
      !hasProviderChat(candidate) &&
      DEV_OPEN.includes(candidate.status),
  );
