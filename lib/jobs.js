...
// A hand-started ⌕ Code review is deliberately one-shot, but what it found
// still needs the same human gate as a review-loop round. Read only the block
// published since this review began (so an empty review cannot revive an old
// round), apply the loop's ordinary already-decided/out-of-diff advice, and
// leave a compact record that survives the review releasing its workspace.
...
export async function holdStandaloneReviewFindings(job) {
  ...
  pushEvent(job, 'info', {
    text: `Code review: ${open.length} finding(s) are waiting in ⚑ Findings. Select what should be implemented or dismiss the findings there.`,
  });
  save(job);
  bus.emit('job', publicJob(job));
  return true;
...
// The dashboard counterpart for a one-shot review's queue card. The review
// itself is already closed and holds no resources; a selected set starts the
// same durable Implement feedback workflow as the PR-board button, with its
// review loop armed for the push that follows. With no selected fixes the
// queue holder is deleted after the verdicts are recorded and the pull
// request is marked code-approved.
...
// The comments typed on a held round, before it is completed: the reason
...
export async function saveReviewFindingsDrafts(id, { verdicts, note, by = 'the user' } = {}) {
  const job = jobs.get(id);
  if (!job || job.kind !== 'devchat') throw new Error('Session not found');
  const held = job.reviewTriage || (job.reviewLoop && job.reviewLoop.triage);
...
export async function triageReviewFindings(id, opts = {}) {
  const job = jobs.get(id);
  if (job && job.reviewTriage) return triageStandaloneReviewFindings(id, opts);
