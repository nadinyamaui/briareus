...
// What a review the loop starts runs on: the reviewer the project was set up
// with, the same setting the board's ⌕ Code review opens on. Reviewing is the
// project's own standard rather than a continuation of the work under review,
// so an automatic round is read by the model Settings chose and not by
// whatever the code happened to be written with.
//
...
// project with no reviewer configured, or one whose row has been deleted or
// deactivated since, falls back to the session's own runtime rather than
// failing every round from here on, and says so once per round.
function loopReviewRuntime(job) {
  ...
  const resolved = configured ? resolveRuntime(configured, getConfig()) : null;
  if (resolved) return { provider: resolved.provider.id, model: resolved.model, effort: resolved.effort };
  if (configured) {
    pushEvent(job, 'info', {
      text: `Review loop: the reviewer this project was set up with is gone, so the review runs on ${job.provider} instead.`,
    });
  }
  return loopSessionRuntime(job);
}
