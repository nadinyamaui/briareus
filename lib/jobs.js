...
async function drainDevQueue(job) {
  for (;;) {
    ...
    // One a killed claude turn had read but never answered is in the
    // transcript already, from when it went in.
    if (!next.sent) pushEvent(job, 'user', { text: next.shown, ...attachmentMeta(next.files) });
...
${zeusRolesBriefing(job)}\n</workspace-context>`;
  ...
  if (ACTIVE.includes(job.status)) {
    ...
    if (input) sendToLiveTurn(job, input, { prompt, shown, files, newFusionBrief });
    else queueDevMessage(job, { prompt, shown, files, newFusionBrief });
    return publicJob(job);
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const armQuiet = () => {
      ...
      quiet = setTimeout(
        ...
        LIVE_INPUT_QUIET_MIN * 60 * 1000,
      );
    };
    if (build.promptVia === 'stream-json') {
      input = {
    ...
    child.on('close', async (code) => {
      ...
      // mid-run. A close drops them along with the rest of the queue.
