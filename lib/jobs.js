...
// migrate, a domain registration) expect it to be there.
async function prepareProfileDatabase(job, profile) {
  if (!profile) return;
  const vars = runVars({
  ...
  const { env } = profileRun(profile, vars);
...
export async function startDevServe(id, { profile: wanted = null } = {}) {
  ...
  if (job.serveProc && job.serveProc.exitCode === null) {
    ...
    if ((job.serveProfile || null) === name) return servedLinks(job, profile);
  ...
  job.serveStarting = startDevServeProc(job, profile).finally(() => {
...
async function servedUrl(job, port, tenant = null) {
  ...
  try {
    ...
    pushEvent(job, 'stderr', {
      text: `Could not publish ${what} through the Cloudflare tunnel: ${e.message}`,
    });
    return local;
  }
}
