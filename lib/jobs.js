...
${zeusRolesBriefing(job)}\n</workspace-context>`;
  ...
  if (ACTIVE.includes(job.status)) {
    ...
    // Behind anything queued before the turn could take input, in order.
    if (input && !devQueues.get(job.id)?.length) {
...
function runDevTurn(job, prompt, { review = false, step = null } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // in as they are sent; settleInput closes it once the turn has answered
    // with nothing left in the background, which is what lets the process exit.
    let input = null;
    ...
    const settleInput = () => {
      ...
      if (job.backgroundTasks.length && !job.turnCanceled && job.status !== 'closed') return;
