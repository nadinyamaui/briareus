...
export async function retryLoopRound(id, { providerId, model, effort } = {}) {
  ...
  if (providerId != null || model != null || effort != null) {
    // What the call left out is filled in from what this loop's reviews
    // already run on, not from the session: the failure notice invites a retry
    // that names only a model, and that model is one of the reviewer's, so
    // defaulting the provider to the session's would drop the model asked for
    // and pin every later round to the session's provider.
    const choice = loopReviewChoice(job);
    const base = choice.runtime;
  ...
  const chosen = loop.reviewRuntime || loop.runtime;
  const where = chosen ? ` on ${chosen.model}` : '';
  pushEvent(job, 'info', {
