...
// Answers whether the details were read at all (a failed query leaves
// everything as the last sync had it).
async function readPrDetails(cfg, repo, number, status) {
  ...
  const failed = (field) => errors.some((er) => Array.isArray(er?.path) && er.path.includes(field));
  ...
  // error (a token without Checks read, a rollup that timed out) is not, and
  // keeps the checks the last sync mirrored.
...
async function syncDevPrNow(job, number = null, { spotted = false, strict = false, details = false } = {}) {
  ...
  try {
    ...
    const status = {
      ...
      detailsEtag: job.prStatus ? job.prStatus.detailsEtag || null : null,
      syncedAt: now(),
    };
    ...
    // keep the query going until they settle, and a check event forces it.
    const before = job.prStatus;
    const etag = pres.etag || null;
    const unchanged =
      !!etag &&
      !!before &&
      before.detailsEtag === etag &&
      before.number === pr.number &&
      before.headSha === status.headSha;
    const checksSettled = !!(before && before.checks) && before.checks.pending === 0;
...
export async function attachPrForBranch(job, { fresh = false } = {}) {
  ...
  try {
    ...
    if (!pr) {
      // A hooked repository announces the pull request the moment it opens
      // (pull_request/opened → syncSessionsOn), so asking again is only for a
      // delivery that got lost.
      const cooldown = repoHasWebhook(job.repo) ? HOOKED_SYNC_MS : BRANCH_PR_COOLDOWN_MS;
      branchPrColdUntil.set(job.id, Date.now() + cooldown);
...
export function repoHasWebhook(repo) {
  ...
  if (!hook.quietSaid) {
    hook.quietSaid = true;
    console.log(`webhooks: ${repo}: no delivery for an hour, back on the minute sync`);
  }
  return false;
}
