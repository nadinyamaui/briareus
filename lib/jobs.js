...
function runDevTurn(job, prompt, { review = false, step = null } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // echo, joined by newlines, so an echo acks every message it carries.
    const ack = (text) => {
      for (const p of input.pending) {
        if (p.acked) continue;
        p.acked = true;
        if (typeof text !== 'string' || !text.startsWith(p.prompt + '\n')) return;
        text = text.slice(p.prompt.length + 1);
      }
    };
        ...
        try {
          for (const e of parser.feed(JSON.parse(line))) {
            ...
            if (e.kind === 'background') {
              ...
              // stdin stays open until that answer is in. It has no echo, and
              ...
              if (input && backgroundTasks.length && !e.tasks.length) {
                input.pending.push({ entry: null, acked: true, wake: true });
              }
            ...
            if (e.kind === 'result' && input) answered();
            const { kind, ...data } = e;
    ...
    child.on('close', async (code) => {
      ...
      await recordTurnUsage(job, turn, prov, model);
