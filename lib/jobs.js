...
function newReviewLoop() {
  return {
    ...
    // there is none, or none that can run" (loopReviewChoice).
    runtime: null,
    ...
    // what makes a provider-level failure of the round
...
function loopReviewChoice(job) {
  ...
  const configured = loop && loop.reviewerFailed ? null : reviewerRuntime(getProject(job.repo));
  const resolved = configured ? resolveRuntime(configured, getConfig()) : null;
  if (resolved) {
    return {
      runtime: { provider: resolved.provider.id, model: resolved.model, effort: resolved.effort },
      from: 'project',
    };
  }
  return { runtime: loopSessionRuntime(job), from: configured ? 'gone' : 'session' };
}
...
async function maybeStartLoopReview(job, { fresh = false } = {}) {
  ...
  try {
    ...
    try {
      ...
      let fallback;
      try {
        fallback = createDevSession({ ...loopSessionRuntime(job), ...spec });
      } catch {
      ...
      pushEvent(job, 'info', {
        text: `Review loop: the reviewer this project was set up with cannot run (${e.message}), so this round and the ones after it run on ${job.provider} instead.`,
      });
      review = fallback;
    }
...
function notifyLoopReviewFailed(review) {
  ...
  failLoopRound(parent, why);
  // A round that died on the project's reviewer is that reviewer failing, not
  // the session's: an account with nothing left in its window, a login that
  // expired. The next push would pick the same one and die the same way, so
  // give the reviewer up for this loop and review that push on the session's
  // own provider rather than reviewing nothing at all.
  const gaveUp = !!parent.reviewLoop.reviewerRound;
  ...
  pushEvent(parent, 'info', {
    text: `Review loop: the code review failed (${why}), so this round reviewed nothing.${
      gaveUp
        ? ` The reviewer this project was set up with is what failed, so the rounds after this one run on ${parent.provider} instead.`
        : ''
