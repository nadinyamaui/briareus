...
// see loopReviewRuntime. `loop.reviewRuntime`, what a retry chose for those
...
function loopReviewChoice(job) {
  ...
  const spec = loop ? loop.reviewRuntime || loop.runtime : null;
  ...
  return { runtime: ownRuntime(job), from: configured && !failed ? 'gone' : 'session' };
}

// The same answer without its provenance: what a retry fills the parts it was
// not given in from, so a retry naming only a model stays on the reviewer.
function loopReviewRuntime(job) {
  return loopReviewChoice(job).runtime;
}
...
async function maybeStartLoopReview(job, { fresh = false } = {}) {
  ...
  try {
    ...
    try {
      ...
      giveReviewerUp(job);
      pushEvent(job, 'info', {
        text: `Review loop: the reviewer this project was set up with cannot run (${e.message}), so this round and the ones after it run on ${job.provider} instead. Repair it in Settings and turn the 🔁 chip off and on to ask the project again.`,
      });
      review = fallback;
    }
...
export async function retryLoopRound(id, { providerId, model, effort } = {}) {
  ...
  if (providerId != null || model != null || effort != null) {
    ...
    const base = loopReviewRuntime(job);
...
function notifyLoopReviewFailed(review, { fromProvider = false } = {}) {
  ...
  pushEvent(parent, 'info', {
    text: `Review loop: the code review failed (${why}), so this round reviewed nothing.${
      gaveUp
        ? ` The reviewer this project was set up with is what failed, so the rounds after this one run on ${parent.provider} instead — repair it in Settings and turn the 🔁 chip off and on to ask the project again.`
        : ''
