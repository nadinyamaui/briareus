  ...
  query($owner: String!, $name: String!, $number: Int!, $head: GitObjectID!) {
    repository(owner: $owner, name: $name) {
      pullRequest(number: $number) {
        ...
        commits(last: 100) { nodes { commit { oid messageHeadline url } } }
...
// missing keeps the last sync's copy, and the caller must not record the ETag
// against it: the skip would then pin that copy (the old head's checks, after
// a push whose rollup read timed out) until the pull request next moves.
...
// The longest the details read is skipped on an unchanged pull request with
// settled checks: what its tag does not cover (a linked issue's state, a CI
// that registers late) is picked up within this.
...
// `details` forces the GraphQL details read even when the pull request itself
// has not changed: a check suite re-run on the same head moves the checks
// without touching the pull request resource.
async function syncDevPr(job, number = null, { spotted = false, strict = false, details = false } = {}) {
  ...
  while (prior) {
    ...
    // forced details read does too: the sync it waited on started before the
    // event and may have skipped them.
    ...
    if (details && next.details) return next;
    prior = next;
  }
  const current = syncDevPrNow(job, number, { spotted, strict, details });
...
async function syncDevPrNow(job, number = null, { spotted = false, strict = false, details = false } = {}) {
  ...
  try {
    ...
    const status = {
      ...
      detailsEtag: before ? before.detailsEtag || null : null,
      detailsReadAt: before ? before.detailsReadAt || null : null,
      // The tag this sync's REST read saw: the next sync's proof that a tag
      // has been seen before (see the details read below).
      etag: pres.etag || null,
      syncedAt: now(),
    };
    ...
    // there is something new to read: a pull request whose ETag is the one
    // this session's last details read was based on has had no push, review
    // or comment since, so the mirrored lists still stand. The session's own
    // tag, not the 304: the ETag cache is shared by every caller and session,
    // so a 304 only says nothing changed since *somebody* last read it. A CI
    // run can move without touching the pull request itself, so pending checks
    // keep the query going until they settle, and a check event forces it. A
    // fresh head with no checks yet is not settled either: its CI may simply
    // not have registered when it was read (CHECKS_REGISTER_MS). And some of
    // what the lists carry moves without the pull request's tag: a linked
    // issue closed or renamed, a CI that posts its first status late. So the
    // skip also expires (DETAILS_MAX_AGE_MS), a slow re-read of an idle one.
    const etag = status.etag;
    const unchanged = !!etag && sameHead && before.detailsEtag === etag;
    const checksSettled =
      !!before &&
      (before.checks
        ? before.checks.pending === 0 &&
          (before.checks.total > 0 || olderThan(before.headSeenAt, CHECKS_REGISTER_MS))
        : !!before.checksUnreadable);
    const fresh = !!before && !olderThan(before.detailsReadAt, DETAILS_MAX_AGE_MS);
    if (details || !unchanged || !checksSettled || !fresh) {
      // The tag is recorded only for a complete read, and only once an earlier
      // sync has seen it too: the sync a webhook sets off runs moments after
      // the event, when GraphQL can still answer from before it while REST
      // already carries the new tag, and recording that read would pin the
      // stale lists until the pull request next moves. Any read that falls
      // short clears the tag instead, so the next sync reads again rather than
      // skipping on a tag the failed read (a forced one on an unchanged pull
      // request, say) left matching.
      const complete = await readPrDetails(cfg, job.repo, number, status);
      if (complete) status.detailsReadAt = now();
      status.detailsEtag = complete && !!before && before.etag === etag ? etag : null;
    }
    job.prStatus = status;
...
export function repoHasWebhook(repo) {
  const hook = hookedRepos.get(String(repo || '').toLowerCase());
...
function syncDevPrs() {
  for (const job of jobs.values()) {
    ...
    // Checks still running keep a merged or closed one watched only for a
    // while after its head was first seen (PENDING_WATCH_MS).
    const pending = !!(job.prStatus.checks && job.prStatus.checks.pending > 0);
    const watch =
      ACTIVE.includes(job.status) ||
      job.prStatus.state === 'open' ||
      (pending && !olderThan(job.prStatus.headSeenAt, PENDING_WATCH_MS));
    if (!watch) continue;
    ...
    // Pending checks keep the minute too: the hook only hears whole suites
    // finish (check_suite), and the panel lists every run as it completes.
    const cadence = ACTIVE.includes(job.status)
      ? 0
      : !pending && repoHasWebhook(job.repo)
        ? HOOKED_SYNC_MS
        : 55_000;
...
// `checks` says the event was about CI (a check suite, a check run, a commit
// status), which can move on a head and a pull request that did not change,
// so the sync reads the details whatever the pull request's ETag says.
export function syncSessionsOn(repo, branch = null, prNumber = null, { checks = false } = {}) {
  ...
  for (const job of jobs.values()) {
    ...
    if (byBranch && !job.prStatus && !sessionPrNumber(job)) {
      ...
      syncDevPr(job, byNumber ? Number(prNumber) : null, { details: checks }).catch(() => {});
    }
  }
  return nudged;
}
