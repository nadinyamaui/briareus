...
function jobEnv(overrides = {}, job = null) {
  ...
  // so it is stripped for the same reason, and so is the OpenAI key voice
  // notes are transcribed with, which bills an account no session should.
  for (const key of Object.keys(env))
    if (key.startsWith('R2_') || key.startsWith('CLOUDFLARE_') || key.startsWith('OPENAI_TRANSCRIBE_'))
      delete env[key];
