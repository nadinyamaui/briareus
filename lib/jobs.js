        ...
        ... on Commit {
          statusCheckRollup {
            contexts(first: 100) {
              nodes {
                __typename
                ... on CheckRun { name status conclusion url }
...
async function readPrDetails(cfg, repo, number, status) {
  ...
  let complete = true;
  if (node.reviews?.nodes && !failed(onPr('reviews')))
    status.reviews = reviewsOf({ reviews: { nodes: rows(node.reviews) } });
  else if (unreadable(onPr('reviews'))) status.reviews = null;
  else complete = false;
  // The commits, for the panel's collapsible list.
  if (node.commits?.nodes && !failed(onPr('commits')))
    status.commitList = commitsOf(rows(node.commits).filter((n) => n.commit));
  else if (unreadable(onPr('commits'))) status.commitList = null;
  else complete = false;
  // The issues this PR closes.
  if (node.closingIssuesReferences?.nodes && !failed(onPr('closingIssuesReferences')))
    status.issues = closingIssuesOf(rows(node.closingIssuesReferences));
  else if (unreadable(onPr('closingIssuesReferences'))) status.issues = null;
  else complete = false;
...
async function syncDevPrNow(job, number = null, { spotted = false, strict = false, details = false } = {}) {
  ...
  try {
    ...
    if (details || moved || checksAwaited(before) || olderThan(before.detailsReadAt, DETAILS_MAX_AGE_MS)) {
      ...
      status.detailsReadAt = complete ? now() : null;
    }
    job.prStatus = status;
    ...
    // this session watched move: either it was awaited on the sha before
    // (running, or a fresh head whose CI had not registered yet), the head
    // moved under it, which is a push it saw land, or the verdict on the same
    // head is not the one the last sync mirrored. That last one is a run whose
    ...
    // and not start. The first sync of a pull request says nothing — a fresh
    // attach and the first tick after a restart both find a finished run that
    // nobody is waiting on any more.
    const rollup = ciChecks(status.checks);
    const settled = rollup && rollup.total > 0 && rollup.pending === 0;
    const verdict = (c) => (c ? `${c.total}/${c.passed}/${c.failed}` : null);
    const watched =
      before &&
      (before.headSha !== status.headSha ||
        checksAwaited(before) ||
        verdict(ciChecks(before.checks)) !== verdict(rollup));
...
function syncDevPrs() {
  for (const job of jobs.values()) {
    ...
    const cadence = ACTIVE.includes(job.status)
      ? 0
      : !awaited && repoHasWebhook(job.repo)
        ? HOOKED_SYNC_MS
        : 55_000;
