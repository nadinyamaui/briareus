...
// but a message handed to a live claude turn only once the CLI reads it (see
// ack in runDevTurn): the answer under way can still end on a question.
function markDelivered(job, entry, live = false) {
  if (entry.newFusionBrief) {
    job.zeusProposalPrompt = null;
    save(job);
  }
  const { awaitingAnswer, lastTool } = job;
  if (!entry.sent) pushEvent(job, 'user', { text: entry.shown, ...attachmentMeta(entry.files) });
  if (live) Object.assign(job, { awaitingAnswer, lastTool });
  else if (entry.sent) Object.assign(job, { awaitingAnswer: false, lastTool: null });
}
...
function runDevTurn(job, prompt, { review = false, step = null } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // answers the wake-up entries added before its init, and no message.
    let inits = 0;
    let wakeUp = false;
    const answered = () => {
      if (wakeUp) {
        wakeUp = false;
        input.pending = input.pending.filter((p) => !p.acked && !(p.wake && p.after < inits));
        return;
      }
      const unread = input.pending.filter((p) => !p.acked);
      ...
      if (unread.length === input.pending.length) unread.shift();
    ...
    // Nothing from the CLI for this long with stdin open and every message
    // answered means something is off, most likely background work it never
    // reported finishing. Closing stdin costs nothing if it is still working:
    // the CLI finishes what it was doing, answers, and exits; new messages
    // just queue meanwhile. A message still waiting on its answer keeps stdin
    // open however long it takes: a foreground build or test suite prints
    // nothing to the stream while it runs.
    ...
    const armQuiet = (ms) => {
      quiet = setTimeout(() => {
        ...
        if (input.pending.some((p) => !p.wake)) return armQuiet(quietMs);
        pushEvent(job, 'info', {
        ...
        try {
          for (const e of parser.feed(JSON.parse(line))) {
            ...
            if (e.kind === 'background') {
              ...
              // stdin if the answer never comes.
              if (input && backgroundTasks.length && !e.tasks.length) {
                input.pending.push({ entry: null, wake: true, after: inits });
              }
