...
async function serveInTurn(job, wanted) {
  ...
  if (same && (fresh.notReady || job.serveRecipe === recipeKey(fresh, profile))) {
    ...
    // started is still up: its links are what the press gets.
    if (fresh.notReady) {
      pushEvent(job, 'info', { text: `Keeping the running app: ${fresh.notReady}` });
    }
    return servedTab(job, profile);
  }
...
// One link per tenant host when the profile names tenants, since each is a
// different app to the Host-header lookup; otherwise the port's one. `first`
// is the tab ▶ opens, and comes back as soon as it is published: every
// hostname queues on the tunnel's one publish chain, a handful of Cloudflare
// round trips each, so the rest reach the session (serveLinks, and through it
// the page) in `all`, once they are in.
function servedLinks(job, profile) {
  const port = job.appPort;
  const proc = job.serveProc;
  const tenants = profile ? profile.tenants : [];
  // A tenant profile publishes only the tenants' hostnames, so {host} (the
  // port's own) would name one with no route behind it unless it goes out too.
  const project = getProject(job.repo);
  const portHost =
    tenants.length && usesPlaceholder(profile, project ? project.runCommands : [], 'host')
      ? servedUrl(job, port)
      : null;
  ...
  const all = (async () => {
    const links = await Promise.all(pending);
    await portHost;
...
async function servedTab(job, profile) {
  const { first } = servedLinks(job, profile);
  return { url: (await first).url, profile: profile ? profile.name : null };
}
...
async function startDevServeProc(job, profile, recipe = null, dir = job.workDir) {
  ...
  const published = servedLinks(job, profile);
