...
import { pickLeastUsedProvider } from './balancer.js';
...
// the daily workspace cleanup removes them; before that, a reused slot only
    ...
    if (loop?.reviewing) {
      ...
      if (!review || !DEV_OPEN.includes(review.status)) {
        if (review?.loopParentId === parent.id) {
          ...
          // A restart is this process dying, not the reviewer the round was
          // riding: the round is retried on it, as the message below says.
...
function pushEvent(job, kind, data, { answers = true } = {}) {
  ...
  // A bubble pushed with answers: false (a message handed to a live turn) is
  // not that yet: it answers once the CLI reads it.
  if (job.kind === 'devchat') {
    if (kind === 'ask') {
...
export function publicJob(job, ledgerUsage = null) {
  ...
  // The links of a server that is up, and only then: the record keeps them
  // across a restart that took the server with it.
  ...
  if (queue && queue.length) {
    // A message a stopped claude turn was handed and never answered is in the
    // transcript already; its chip says it is going in again.
  ...
  // A claude turn still reading its stdin takes the next message now, not
  // when the turn ends; the composer says which of the two it is.
  ...
  if (job.kind === 'devchat') {
    ...
    meta.canCompact = job.status === 'idle' && !!compactTarget(job);
  }
  return meta;
}
  ...
  try {
    ...
    if (parentId) {
      ...
      try {
        ...
        // A usage projection must never prevent the underlying session from
        // being deleted. Already-absorbed estimate metadata still transfers.
...
// The environment every job child process gets: the parent's less the
// server's own credentials (childEnv), plus git's GitHub credentials, plus
// whatever the caller overrides, plus the job's PHP version pinned ahead of
// everything else on PATH so it runs against that rather than the machine's
// globally selected one (a no-op when neither pin is set).
...
// A claude turn reads its stdin until it has answered with nothing left in the
// background. While it is `taking`, a message sent goes straight into that
// CLI, mid-turn or while its sub-agents and Monitors are still out, instead of
// waiting in devQueues for all of it to finish. The input stays here for the
// turn's whole life, also once it stops taking messages: what it was handed
// and has not answered still counts against MAX_QUEUED, since a stopped turn
// puts it back in the queue.
const turnInputs = new Map(); // job id -> { taking, pending, send(entry) }
// How long a turn holding stdin open may stay silent before it is closed
// anyway (see runDevTurn).
const LIVE_INPUT_QUIET_MIN = 30;

// The first workspace preparation for a newly-created session. Most callers
// fire and forget, because the conversation streams its progress into the UI;
// ▶ Run on a pull-request card waits for this promise before it can start the
// app server in that checkout. Process-local like the message queue: a restart
// marks the session interrupted and there is no preparation left to await.
const devStarts = new Map(); // job id -> Promise<void>

const MAX_QUEUED = 20;

// Refuse a message past MAX_QUEUED: the queue, plus whatever a claude turn was
// handed live and has not answered yet.
...
export function dropQueuedMessage(id, index) {
  ...
  // Its bubble went into the transcript when a stopped turn was handed it;
  // the transcript has to say it was never answered after all.
  if (dropped.sent) {
    pushEvent(job, 'info', {
...
async function drainDevQueue(job) {
  for (;;) {
    ...
    markDelivered(job, next);
    if (job.status !== 'running') setStatus(job, 'running', { error: null });
    await runDevTurn(job, next.prompt);
  }
}

// A message is going to the agent now: the transcript gets its bubble, and a
// new Zeus brief resets the proposal round. One a killed claude turn had read
// but never answered is in the transcript already, from when it went in.
// Whatever the user sends answers an open question (the bubble clears it),
// and starts a new proposal round, but a message handed to a live claude
// turn does either only once the CLI takes it in (see ack in runDevTurn):
// the answer under way can still end on a question, or spawn its second
// proposal model.
function markDelivered(job, entry, live = false) {
  if (!live) deliverFusionBrief(job, entry);
  if (!entry.sent)
    pushEvent(job, 'user', { text: entry.shown, ...attachmentMeta(entry.files) }, { answers: !live });
  else if (!live) Object.assign(job, { awaitingAnswer: false, lastTool: null });
}

// Once per brief: a round the new brief started must keep its shared prompt.
function deliverFusionBrief(job, entry) {
  if (!entry.newFusionBrief) return;
  entry.newFusionBrief = false;
...
// A turn about to be killed (Stop, a close, its time limit) takes no more
// messages: one sent from here on queues for the next turn instead of going
// into a process that is exiting. What it had been told and not answered yet
// is put back in the queue after a Stop, and dropped with a note after a
// timeout (see runDevTurn).
...
const ZEUS_PROPOSAL_ROLES = ['product', 'architecture'];
...
const ZEUS_ROLES = [...ZEUS_PROPOSAL_ROLES, 'qa', 'validator'];
...
export function createDevSession({
  ...
  if (!parentId && !loopParentId && !qaParentId && !loopFixParentId) assertAcceptingWork();
  const cfg = getConfig();
  ...
  // Whatever named the provider (the composer, a board errand, a project's
  // reviewer or worker runtime, an orchestrator's spawn) named an account
  // family as much as an account: the session runs on the member of that
  // family with the most quota left. Decided once, here: a conversation lives
  // in one login's session store and never moves.
  ...
  // A PR preview is a normal prepared workspace with no opening agent turn:
  // it exists so the board can serve a clean checkout without spending model
  // tokens. Once open, it is still a regular conversation and the selected
  // provider is what a later message resumes on.
  preview = !!preview;
  if (preview && (orchestrator || review || qa || local || autoClose || reviewLoop || qaLoop)) {
    throw new Error('A pull request preview only prepares and serves its worktree');
  }
  if (preview && !branch) throw new Error('A pull request preview needs the branch to run');
  ...
  // session's effort into the provider's review command. Codex effort support
  // belongs to the selected model, not the provider as a whole.
  ...
  const job = {
    ...
    chats: {},
    // Which account each step's provider was balanced onto for this session,
    // keyed by the row id the project configured (see stepProvider). Decided
    // on the step's first turn and kept, so the step's conversation stays on
    // one login.
    stepProviders: {},
    turns: 0,
    costUsd: null,
    ...
    // Findings from a code review started by hand are held here after the
    // review has published and released its workspace. Loop reviews put the
    // same shape on their durable parent (`reviewLoop.triage`); a standalone
    // review has no such parent, so this small record keeps its findings on
    // the shared ⚑ screen until the user dismisses them or starts an
    // Implement feedback session with the selected set.
    reviewTriage: null,
    ...
    // Created by ▶ Run on the pull-request board. No agent turn was needed to
    // prepare it, and another click can reuse it while it is still pristine.
    preview,
    // When sweepExpiredPreviews closes and deletes it. Pushed forward by every
    // ▶ Run click that reuses it; stored so a restart still honours it.
  ...
  if (preview) {
    pushEvent(job, 'info', {
      text: `Preparing pull request${prNumber ? ` #${prNumber}` : ''} on ${branch} to run locally. Unless somebody chats in it, this session closes and deletes itself ${PREVIEW_TTL_MS / 60_000} minutes after the last ▶ Run.`,
    });
  } else {
...
// Prepare and serve a pull request without paying for a throwaway agent turn.
// Every click starts from scratch: a pristine preview already on this pull
// request is closed and deleted first, so the app comes up on a fresh clone of
// the current head and a database the setup commands just filled, rather than
// whatever the last run left behind. Once somebody chats in a preview it is
// their session, not an environment, and it is left alone.
export async function startPullRequestPreview({ provider, model, effort, repo, branch, prNumber, title }) {
  const stale = [...jobs.values()].filter(
    (candidate) =>
      candidate.kind === 'devchat' &&
      candidate.preview &&
      candidate.repo === repo &&
      candidate.startedOnPr === prNumber &&
      !hasProviderChat(candidate),
  );
  for (const old of stale) {
    // Closing mid-setup would leave the start to finish claiming a clone and a
    // database for a session that is gone, so let it land first.
  ...
  if (job.status !== 'idle') throw new Error(job.error || 'The pull request workspace could not be prepared');
  const served = await startDevServe(job.id);
  // The ten minutes count from the last click, not from the first: whoever
  // just opened the app is looking at it now.
  job.previewExpiresAt = Date.now() + PREVIEW_TTL_MS;
  save(job);
  return { ...served, session: publicJob(job) };
}

// A ▶ Run preview holds a clone slot, a database server and an app port for
// something people glance at and walk away from, so it expires on its own
// instead of waiting for somebody to close it. Only a pristine one: once
// somebody chats in it, it is a conversation like any other and stays until
// closed by hand. One still preparing is left alone too; closing it would
// fail the click that is waiting on it.
...
//
// It serves the run profile the user was looking at: the one this session
// served, or, for a run the QA loop started, the one its parent served.
...
function startDevSession(job, prompt) {
  const starting = (async () => {
    try {
      ...
      // A 🎬 test run started from the board serves the default run profile
      // (lib/prtasks.js), whose database has to exist before its first turn
      // does; the QA loop's own run sees to that itself (runTestRunTurn).
      if (job.activity === 'test-run') {
        await prepareProfileDatabase(job, pickRunProfile(getProject(job.repo), null)).catch((e) =>
          pushEvent(job, 'stderr', { text: e.message }),
        );
      }
      // A preview has reached its whole purpose once the checkout and database
      // are ready. startPullRequestPreview is waiting to serve it; no provider
      // process is spawned and no usage is recorded.
      if (job.preview) {
        setStatus(job, 'idle');
        return;
      }
      ...
      // A review somebody started from the pull-request board has no durable
      // loop parent to receive its result. Read its declared findings before
      // the auto-close releases the clone, and retain only a queue card when
      // there is something for the user to decide.
      ...
      // Which stage this failure came out of, read before the status is
      // overwritten: 'running' is the provider turn itself, so an exhausted
      // account or a CLI exiting non-zero lands here as one. 'preparing' is
      // everything around it — the clone, the project's setup steps, the
      // database server — which fails for reasons of its own and is no
      // evidence about the provider (see notifyLoopReviewFailed). The turn's
      // own time limit is the exception inside 'running': the timer kills the
      // CLI, so the turn rejects on a null exit code while a provider that was
      // working fine was simply given a job too big for the limit. A turn the
      // CLI itself reported failed while exiting cleanly is the other one: the
      // process ran to the end and its stream said the work failed (a tool
      // error, a refusal, a content-length stop), which says nothing about the
      // account behind it (runDevTurn).
...
// A step's provider is a family of interchangeable accounts like any other
// (see lib/balancer.js), so the step runs on the member with the most
// headroom rather than for ever on the row whose id the project stored: an
// account at its limit would otherwise fail every test-sheet and test-run
// turn while whole sessions balance away from it.
//
// Balanced once and remembered, not per turn: each member holds its own
// conversation for this workspace (providerChat above), so re-picking mid-run
// would leave the step resuming a thread on one account with the transcript
// on another. The session's own account wins when the step names its group,
// which is what keeps a step configured to the session's provider running in
// the session's own conversation.
//
// The pin is remembered under the row id the project configured, not under
// the group key: that key carries the members' catalogs, which move when the
// CLI refreshes its model cache or the operator edits a row, and a pin filed
// under a key nobody computes any more would silently re-balance the step
// onto an account with none of its conversation. The pinned member still has
// to be interchangeable with the configured row as things stand now.
...
async function reopenWorkspace(job) {
  ...
  await prepareDevWorkspace(job, { preserve: true });
  if (job.status === 'closed') throw new Error('closed');
}
...
// A follow-up message. Mid-turn it goes straight into a claude turn still
// reading its input, and joins the queue otherwise; on an idle session it runs
...
export function sendDevMessage(id, text, attachments, zeusRoles) {
  const job = jobs.get(id);
  if (compacting.has(id)) throw new Error('Wait for context compaction to finish before sending a message');
  if (!job || job.kind !== 'devchat') throw new Error('Session not found');
...
${zeusRolesBriefing(job)}\n</workspace-context>`;
  ...
  const entry = { prompt, shown, files, newFusionBrief };
  ...
  // Messages still queued from before (a reopen that failed on them) go
  // first, and this one behind them.
  const behindQueue = !!devQueues.get(job.id)?.length;
  if (behindQueue) queueDevMessage(job, entry);
  else markDelivered(job, entry);
  ...
  (async () => {
    ...
    try {
      ...
      if (!behindQueue) await runDevTurn(job, prompt);
      await drainDevQueue(job);
...
//
// A run profile goes in front: its own commands ahead of the run commands, and
// its env over the session's (which is how a vertical read from process env at
// boot gets chosen, since a real env var beats the checkout's .env).
//
...
function devServeRecipe(job, profile) {
  ...
  // Saved profiles that stopped reading (a rule added since they were saved)
  // pick no profile, and serving the plain run commands would hide that.
  ...
  // A local checkout runs against the database its own .env names. Without a
  // name, {database} would render empty and DB_DATABASE={database}_x boot the
  // app against a database called _x, so that is refused instead.
  ...
  // A tenant shares the port's DNS label, which DNS caps at 63 characters.
  // Past that every publish of it fails, and its before-commands would
  // register a hostname nothing answers on, so it is refused up front.
...
// Whether the run commands or the profile's commands and env values use a
// {token}.
...
// A profile's DB_DATABASE naming a database other than the session's own gets
// it created first, on the session's server; the profile's commands (a
// migrate, a domain registration) expect it to be there. `recipe` is ▶ Run's,
// which already rendered the profile's env and {database}; the callers without
// one get both worked out here.
async function prepareProfileDatabase(job, profile, recipe = null) {
  ...
  // The session's own database (a local checkout's included, whose name only
  // its .env knows) is already there: nothing to make.
  if (!env.DB_DATABASE || env.DB_DATABASE === database) return;
  // A profile that points the app at another server (or engine) would get
  // its database made where the app never looks, so it is left to whoever
  // runs that server.
...
// `profile` names the run profile to serve. Without one, the session serves
// the profile it served last, or the project's default: the plain click on ▶.
// A different profile than the one running is a restart, since the vertical
// is read once at boot and the tenants' hostnames differ.
//
// Every start and switch of a session queues behind the one before it and
// reads the session afresh when its turn comes: two ▶ presses never spawn two
// servers (a second spawn would overwrite serveProc, leaving a detached tree
// nothing can ever kill, squatting on a pool port), a second press asking for
// the same profile finds it running, and a plain press during a switch finds
// the profile switched to rather than undoing the switch.
//
// The queue covers the stop and the spawn only. The tab's publish waits on the
// tunnel's one chain, behind every other session's hostnames, so it is awaited
// outside: a switch picked meanwhile goes ahead without waiting on Cloudflare.
export async function startDevServe(id, { profile: wanted = null } = {}) {
  ...
  assertServable(job);
  const turn = (job.serveQueue || Promise.resolve()).then(() => serveInTurn(job, wanted));
...
async function serveInTurn(job, wanted) {
  ...
  // A live server may sit on a shifted port, so reuse the port it bound
  // rather than recomputing the preferred one. The same profile edited in
  // Settings since the server started is a restart all the same: its env and
  // before: commands only reach the app at boot, and its links would name
  // tenants the running app never registered.
  const fresh = devServeRecipe(job, profile);
  if (same && (fresh.notReady || job.serveRecipe === recipeKey(fresh, profile))) {
    // Settings that no longer build a recipe (run commands blanked, a local
    // .env that lost DB_DATABASE) cannot restart anything, and the app they
    // started is still up: its links, from the tenants it started with rather
    // than the ones the edited profile lists, are what the press gets.
    if (fresh.notReady) {
      pushEvent(job, 'info', { text: `Keeping the running app: ${fresh.notReady}` });
    }
    return servedTab(job);
  }
  // Whatever can refuse the new profile (its {database} guard, its database)
  // is settled while the old app is still up, so a refusal leaves it serving.
  // The recipe renders the port the old server holds, which the stop frees.
...
// Where the browser should open a served port. The dashboard is often reached
// through a tunnel, and 127.0.0.1 from there is the viewer's own machine, so
// with a Cloudflare tunnel configured the port is published through it. When
// that fails the app is still up, so the local URL goes out with the reason
// in the log rather than the whole Run failing.
async function servedUrl(job, port, tenant = null) {
  ...
  try {
    ...
    // The before: commands registered the tenant under the name {host:<tenant>}
    // rendered to, the published one; the app knows no tenant by the local name.
...
// The hostnames a server started with `profile` answers on: one per tenant
// when the profile names tenants, since each is a different app to the
// Host-header lookup, otherwise the port's own. A tenant profile publishes
// only the tenants' hostnames, so {host} (the port's own) would name one with
// no route behind it unless it goes out too.
...
// The links of the running server, from the hostnames it started with (not
// the profile as Settings read now). `first` is the tab ▶ opens, and comes
// back as soon as it is published: every hostname queues on the tunnel's one
// publish chain, a handful of Cloudflare round trips each, so the first
// tenant goes at its head and the rest reach the session (serveLinks, and
// through it the page) in `all`, once they are in, and to `onLive` while the
// server they belong to still runs. `all` never rejects, `onLive` included: a
// caller that gave up on it (a start whose run commands exited at once) must
// not leave an unhandled rejection, which takes the whole server down.
...
// What ▶ answers with: the tab to open, still publishing. The other links
// follow on their own.
function servedTab(job) {
  return { first: servedLinks(job).first, profile: job.serveProfile || null };
}

// A close (or a failed turn) that lands while a start is still awaiting, a
// switch stopping the old server above all, kills nothing: there is no server
// yet. The start checks the session is still open in the workspace it began
// in at every step instead, and gives up rather than creating a database after
// the close dropped them, or spawning a server nothing will ever kill in a
// clone handed back to the pool.
...
async function startDevServeProc(job, profile, recipe = null, dir = job.workDir) {
  ...
  // Publishing a port for the first time is a few API round trips; it runs
  // alongside the server's start instead of ahead of it.
...
// A switch of profile starts the next server on the same port, so the old tree
// has to be gone first: a port still held would shift the new one, and every
// hostname with it. Bounded, since a tree that ignores SIGKILL is the port
// probe's problem to route around, not a reason to hang the click.
async function stopDevServe(job) {
  ...
  // The shell's exit is not the server's: the app is its child, killed in the
  // same breath but let go of the port a moment later.
...
function compactTarget(job) {
  ...
  if (usage?.source !== 'codex') {
    ...
    if (
      ...
      !canResume('codex', sessionId)
    )
      return null;
    return { provider, sessionId, model: job.model };
  }
  const provider = getProvider(usage.providerId);
  if (!provider || provider.binary !== 'codex' || !usage.sessionId) return null;
...
export async function compactDevSession(id) {
  ...
  if (!target) throw new Error('No Codex context is available to compact');
  const { provider: prov, sessionId, model } = target;
  const cfg = getConfig();
  const found = getBinary('codex').bin(cfg);
  ...
  job.contextUsage = { ...job.contextUsage, source: 'codex', providerId: prov.id, sessionId, model };
  ...
  job.turnCanceled = false;
  setStatus(job, 'running');
  pushEvent(job, 'info', { text: 'Compacting Codex context…' });
  try {
    ...
    pushEvent(job, 'info', { text: 'Codex context compacted.' });
  } catch (error) {
    pushEvent(job, 'info', { text: `Codex compaction did not complete: ${error.message}` });
    throw error;
  } finally {
    ...
    try {
      await recordTurnUsage(job, { inputTokens, outputTokens, durationMs }, prov, model);
    } catch (error) {
...
export async function closeDevSession(id) {
  ...
  // A round held for the ⚑ Findings screen goes the same way: a closed session
  // has no fix session to send verdicts to, and a card left standing would
  // record them on a pull request nobody is working any more.
  dropHeldRound(
    job,
  ...
  killJobProcess(job);
  killDevServe(job);
  // The status only turns closed once the databases below are dropped, and a
  // ▶ Run start resuming during those awaits (a switch whose stop just ended,
  // one queued behind it) would find the session still open in its workspace
  // and spawn after the only kill above. serveCalledOff reads this instead.
  job.closing = true;
  try {
    await releaseInstance(job);
    // The session's own database goes back with the rest of what it held. A
    // reopen makes it again and the project's setup commands fill it, so the
    // only thing keeping it would be the disk (or, on a pool server, the RAM)
    // it sits in and the table definitions it costs every instance on that server.
...
function newReviewLoop() {
  return {
    ...
    // (retryLoopRound), kept so every later round — and the fix sessions and
    // the QA run that continue the work — stays off the provider that failed
    // this one. Only a retry that named a provider_id outright writes it:
    // moving a whole task onto another account is what naming one means. Null
    // means "whatever the session it belongs to runs on" (loopSessionRuntime)
    // for a fix session and the QA run; a review reads it as "the reviewer
    // this project was set up with, and the session only when there is none,
    // or none that can run" (loopReviewChoice). Dropped when it names the
    // reviewer this loop has just given up (giveReviewerUp): the review falls
    // back off that provider, and a fix session must not stay pinned to it —
    // which is a move somebody made, so the give-up says it was undone.
    runtime: null,
    // The same, for the loop's reviews alone: what a retry that named only a
    // model or an effort settled on, the parts it left out filled in from what
    // the reviews already ran on — which may be the project's reviewer. Kept
    // apart from `runtime` because such a retry chose nothing for the fix
    // sessions or the QA run, and writing the reviewer into the loop-wide
    // override would open them on a provider nobody pointed them at. It
    // outranks `runtime` for a review, being the later and more specific
    // answer; loopReviewChoice reads either as riding the project when it
    // names the project's reviewer. A retry that named the project's own
    // reviewer by provider_id writes it too, on that reviewer's configured
    // model and effort: what such a call leaves out is the setting's answer
    // for a review, while `runtime` keeps the session's for the work it moves.
    reviewRuntime: null,
    // Whether the provider frozen into `reviewRuntime` above came from the
    // project's ⌕ Code review setting rather than from an override somebody
    // named. Such a retry chose a model, not a provider, so the row it froze
    // in is the setting's answer of the moment: repointing ⌕ Code review at
    // another provider in Settings — what the give-up messages ask for — drops
    // it (loopReviewChoice), and the reviews move with the setting instead of
    // staying on a row nothing names any more.
    reviewRuntimeFromProject: false,
    // Set once the project's reviewer has proved unusable for this loop: the
    // row resolves, but no session could be created on it, or the round that
    // did start died on its provider. From then on the rounds run on the
    // session's own runtime — which the push under review demonstrably came
    // out of — rather than failing every round on a reviewer nothing here can
    // repair. Two things clear it: repairing the reviewer and turning the 🔁
    // chip off and on, which builds a new loop (newReviewLoop) that asks the
    // project again, as both give-up messages say; and a retry naming that
    // same reviewer by provider_id, which is the operator saying it is
    // repaired (retryLoopRound). Setting it takes any override naming that
    // reviewer down with it, so the fix sessions leave it too (giveReviewerUp).
    reviewerFailed: false,
    // Whether the round now running started on that configured reviewer. It is
    // what makes a failure of the round that came out of the provider
    // (notifyLoopReviewFailed) set the flag above, instead of blaming a
    // reviewer that had nothing to do with it.
    reviewerRound: false,
    ...
    discoveryRetries: 0,
    // Whether "the reviewer this project was set up with is gone" has been
    // said. Said once for the life of the loop, like discoveryErrorSaid above:
    // the deleted row stays deleted, and the sibling notice for a reviewer
    // that cannot run is once-only by way of reviewerFailed.
    goneSaid: false,
  };
}
...
// A fix session and a QA run of the loop are continuations of that session's
// task, so they follow the runtime it was given: an orchestrator that moved a
// worker onto another provider (because the first one's account was out of
// quota, say) is deciding for that worker's fixes too. The one thing above it
// is `loop.runtime`, the override a retry moved this loop onto
...
// The loop's reviews are the exception, and follow the project instead:
// see loopReviewChoice. `loop.reviewRuntime`, what a retry chose for those
// reviews alone, is deliberately not read here: a retry that named a model for
// the review said nothing about what writes the fixes.
...
// What the session itself was created on, with no override over it: the
// runtime the push under review demonstrably came out of, and so the loop's
// last resort when the reviewer it was asked to use cannot review anything.
function ownRuntime(job) {
  return { provider: job.providerId, model: job.model, effort: job.effort };
}

// What a review the loop starts runs on, and where that answer came from: the
// reviewer the project was set up with, the same setting the board's ⌕ Code
// review opens on. Reviewing is the project's own standard rather than a
// continuation of the work under review, so an automatic round is read by the
// model Settings chose and not by whatever the code happened to be written
// with.
//
// A retry's override still wins: it is what moved this loop off a provider
// that failed it, and the project's reviewer may well be that provider. A
// project with no reviewer configured, one whose row has been deleted or
// deactivated since, and one whose reviewer has already proved unusable to
// this loop (reviewerFailed) all fall back to the session's own runtime rather
// than failing every round from here on. `from` says which of the four it was,
// so the round can say it once and knows whose failure a refused start is.
//
// `from` follows the provider the round actually rides, not the field the
// answer came out of: a retry that named only a model or an effort had its
// provider filled in from the reviewer (retryLoopRound writes that one to
// `reviewRuntime`), so an override can be the project's reviewer without
// anybody having chosen it, and reading that as "somebody moved this loop off
// the project" would silently switch off both safety nets around a failing
// reviewer.
function loopReviewChoice(job) {
  ...
  // A retry that named only a model or an effort had its provider filled in
  // from the ⌕ Code review setting, so the row frozen into `reviewRuntime` is
  // that setting's answer of the moment and not a provider anybody chose. When
  // Settings repoints the reviewer elsewhere — exactly what the give-up
  // messages ask for — the reviews follow it: left pinned, the loop would
  // review on the old row while Settings names another, with `from: 'override'`
  // switching off both the give-up and the start-refusal fallback for a
  ...
  // What a retry chose for the reviews alone first, then the loop-wide
  // override a retry that named a provider outright moved the whole task onto.
  ...
  // An override naming the reviewer this loop has already given up is no more
  // usable than the setting it was filled in from, so it is dropped like the
  // setting: keeping it would fail every round on the wall this loop has
  // already hit, with the fallback below skipped for being an override.
  if (override && !(overrideIsReviewer && failed)) {
    return {
      ...
      // Riding the reviewer through an override is not the same as reading the
      // setting: what a partial retry records (reviewRuntimeFromProject), so
      // only a provider the setting itself supplied moves when it changes.
      // The review override the setting supplied keeps the provenance it
      // already has, though: a second partial retry re-reads this answer and
      // stores it, and a flat false there would unmark a row nobody chose and
      // freeze the reviews on it for good.
  ...
  // The session's own runtime rather than loopSessionRuntime's: the override
  // that was just dropped is the reviewer this loop gave up, and it must not
  // come back through the fallback.
  return { runtime: ownRuntime(job), from: configured && !failed ? 'gone' : 'session', setting: false };
}

// What the project's ⌕ Code review names right now, resolved, or null when it
// names none, or one that no longer resolves. What a retry reads to tell "run
// on the project's reviewer again" from "move this loop somewhere else", and
// what a retry that turns out to be the former puts the reviews back on.
...
// The project's reviewer has proved unusable to this loop: it refused to start
// a round, or the round that started died on its provider. Recording that is
// only half of it — a retry may have named that same provider outright, and
// then `loop.runtime` points the loop-wide override (loopSessionRuntime: the
// fix sessions and the QA run) at exactly what the review has just given up.
// Left there, a round falls back and publishes findings that
// startLoopFixSession then refuses to implement, round after round: the
// findings land on the pull request, no fix session pushes, and the loop idles
// for good. So an override naming the given-up reviewer goes with the setting
// it names, and the whole loop is back on the session's own runtime — the one
// the push under review demonstrably came out of.
//
// Dropping the loop-wide one is not free, though: only a retry that named a
// provider outright writes it (retryLoopRound), which is how a whole task is
// moved off an account that is out of quota, and the runtime it falls back to
// is the session's own — possibly the very account that retry escaped. It goes
// either way (a provider that cannot review cannot write the fixes), so this
// answers with the label it dropped and the give-up messages say the move was
// undone, rather than sending the fix sessions back in silence.
...
// What that costs the party who moved the loop: the fix sessions and the QA
// run it moved are back on the session's own runtime, and only a retry naming
// another provider moves them again. Empty when nothing loop-wide was dropped,
// which is every give-up on a loop nobody had moved.
...
// What the give-up messages' way out costs when a QA loop is queued behind
// this one:
// turning the 🔁 chip off drops the QA loop with it (setReviewLoop), and
// turning it back on rebuilds the review loop alone. The operator sent to
// Settings by a give-up has to know to arm 🎬 again, or the loop converges and
// the QA that was waiting never runs.
...
export function setReviewLoop(id, on) {
  ...
  if (on) {
    ...
    if (running) {
      ...
      // The head that review is reading: what the loop that started it
      // recorded on it (reviewedSha), or, for a review started from the board,
      // the head now, since nothing recorded what it checked out. The hold
      // (holdForTriage) carries it as the sha the round was reviewed at, and
      // the commit gate compares against it. Left unset, the round could never
      // be flagged stale and a nothing-to-fix send would re-review this same
      // commit.
...
// Takes a held round off the ⚑ Findings screen when its session or its pull
// request is no longer there to send it to. Nothing is recorded: what the round
// found is on the pull request, and the verdicts it waited for are moot. Only
// mutates; the caller saves and projects the job with whatever else it changed.
...
// Whether the branch has moved past the commit a held round was reviewed at.
// The hold's own sha is the one field both the stale mark and the Send gate
// read, and a hold that recorded none (the probe answered nothing at the
// time) counts as not moved: it cannot be told apart from an unmoved one,
// and treating it as moved would re-review the same commit on every send.
// The head comes from loopHeadSha, which sees a push from anywhere: the
// clone's tracking ref, or the pull request's mirrored head when that names
...
// A push landing while a round is held is one the round never saw. The hold
// carries the sha it was reviewed at (holdForTriage); when the branch has
// moved past it the card says so, so the person ruling on the round knows the
// findings may already be fixed by hand, and a Send that fixes nothing
// re-runs the commit gate instead of converging on the unreviewed head.
// Only mutates, and answers whether it did; the caller saves and projects.
...
// races this settle. The mirror is the fallback for a branch this workspace
// has no tracking ref for, and the answer when it names a commit the clone
// has never seen: that is a push made from somewhere else (by hand, from
// another checkout) that no fetch has brought in yet, so the mirror is ahead
// of the tracking ref rather than behind it, and the ref would answer a head
// the loop already reviewed.
...
// Whether the session's clone has the commit at all: what tells a mirrored
// head that is ahead of the tracking ref (a push from elsewhere, not fetched
// yet) from one that is behind it (the clone's own push, not synced yet).
...
async function maybeStartLoopReview(job, { fresh = false } = {}) {
  ...
  // A round on hold survives a restart as it was: the decision it waits for
  // is a person's, and nobody's absence makes it. A push in the meantime is
  // noted on the card, not reviewed: one round on the screen at a time.
  if (loop.triage) {
  ...
  // Said once, like the loop's other standing conditions: the row stays
  // deleted, and one identical line per push is noise for the life of the loop.
  ...
  try {
    ...
    try {
      ...
      // The configured reviewer resolved, and still cannot run: its CLI is not
      // installed on this machine, its family has no member left to start on.
      // Nothing this loop does repairs that, and failing every round on it
      // leaves every push unreviewed until somebody retries by hand, so drop
      // back to the session's own runtime — the one the push under review came
      // out of, so it works — and stay there for the rounds after this one.
      if (choice.from !== 'project') throw e;
      let fallback;
      try {
        // ownRuntime, not loopSessionRuntime: what refused may be an override
        // that is the reviewer itself (loopReviewChoice), and falling back
        // onto it would refuse the round all over again.
        fallback = createDevSession({ ...ownRuntime(job), ...spec });
      } catch {
        // Refused on the session's runtime too, so the reviewer is not what is
        // wrong: a full pool turns everyone away. Report the first refusal, as
        // this always did, and leave the project's reviewer to the next round.
        throw e;
      }
    ...
    // Whether that reviewer is what this round is riding on, which is what
    // makes its provider dying mid-round (notifyLoopReviewFailed) give it up.
    ...
    // The commit this round reads, on the review itself: a loop armed again
    // while it runs adopts it (setReviewLoop) with the head it actually
    // checked out rather than the head at arm time.
...
async function resolveLoopRound(parent) {
  ...
  // Nothing is fixed until somebody has read the round: it is held for the
  // dashboard's ⚑ Findings screen, and only what is marked fix there goes on.
  await holdForTriage(parent, prNumber, findings);
}

// Hold a round for the person at the dashboard. Every round stops here, the
// orchestrator's worker as much as a session started by hand: the reviews
// report, and a fix session costs a round of tokens and another review, so
// which findings are worth that is a decision the ⚑ Findings screen exists
// to make by hand rather than one the loop takes on its own. The findings are
// sorted the way the loop's own rules would sort them, and the split goes
// along as advice rather than as verdicts: what the rules would have parked is
// marked so, and the person decides. Nothing is recorded and no fix session
// starts until they do (triageLoopFindings). The round cap stands where the
// fix session would start (startRoundFix), not here: a final round is held
// and ruled on like any other, so what it found is recorded on the pull
// request either way, and a nothing-to-fix send converges the loop the way
// the cap never stood in the way of on rounds that fixed nothing.
async function holdForTriage(parent, prNumber, findings) {
  const loop = parent.reviewLoop;
  let sorted;
  try {
    sorted = await sortFindingsForFix(parent.repo, prNumber, findings, {
      severityFloor: roundSeverityFloor(loop),
    });
  } catch (e) {
  ...
  // The loop this round belongs to may be gone too: 🔁 off (or off then on)
  // during the read replaces parent.reviewLoop, and a hold written onto the
  // detached object would be announced everywhere and shown nowhere.
...
// Whose pull request a review is of. This, and not how the review was
// started, is what decides what its ⚑ Findings card offers: on your own pull
// request the findings are yours to send to a fix session, and on somebody
// else's they are theirs to answer, so the card only replies on the threads
// and takes the review off the queue.
//
// The account this app acts as is the project's configured PR author when it
// has one (a project may post as a bot or as a colleague's account) and the
// token's own user otherwise, so an unconfigured project still recognises its
// own work. Anything unknown — no token, an author GitHub would not name, a
// lookup that failed — answers false: the card that rules nothing and spends
// nothing is the safe one to be wrong with.
...
// A hand-started ⌕ Code review is deliberately one-shot, and what its card
// then offers follows whose pull request it reviewed (pullRequestAuthor).
// On somebody else's, the card is a queue card and not a gate: it says the
// review has not been read yet, and what it offers acts on the review rather
// than ruling on it — a reply on a finding's thread (replyToReviewFinding),
// deleting one from the review (deleteReviewFinding), and Complete
// (completeStandaloneReview). On the user's own pull request it is the same
// gate a review-loop round gets: a verdict per finding, and Complete sends
// what was marked fix to an Implement feedback session
// (triageStandaloneReviewFindings).
// Read only the block published since this review began (so an empty review
// cannot revive an old round), apply the loop's ordinary
// already-decided/out-of-diff advice, and leave a compact record that survives
// the review releasing its workspace.
// Calling this twice is harmless: an answered review can pass through both
// settle paths, and a held card must never be replaced underneath the user.
...
// The loop has nothing left to implement: the round is closed as the end of
// the loop's work, said in the session, to the orchestrator and to the
// dashboard, and the QA run queued behind the loop is its next step. `why`
// is the reason in the past tense, read both ways.
...
// The orchestrator is told the round is on hold, not asked to rule on it:
// the loop it armed is standing still, and reporting the task done or
// sending the worker more work in the meantime would be judging code that a
...
// The verdicts on the round a loop is holding (holdForTriage): the person at
// the ⚑ Findings screen, or an orchestrator the user told to rule for them.
...
// guidance. Nothing to fix adds the code-approved label and converges the
// round.
/**
 * @param {string} id
 * @param {{ verdicts?: any[], note?: string, by?: string }} [opts]
 */
export async function triageLoopFindings(id, { verdicts, note, by = 'the orchestrator' } = {}) {
  const job = jobs.get(id);
  if (!job || job.kind !== 'devchat') throw new Error('Session not found');
  const loop = job.reviewLoop;
  ...
  // Nowhere to send the verdicts: a closed session and a merged or closed
  // pull request drop their round (closeDevSession, the PR sync), so these
  // catch the record that predates that and the merge the sync has not seen
  // yet, before a triage comment lands on a pull request nobody is fixing.
  ...
  // One send per round: the record and the comment below are a round-trip,
  // and two callers that both saw the round held (two dashboard tabs, the
  // user and an orchestrator) would each write verdicts and each post a
  // triage comment before either noticed the other. The second is refused
  ...
  try {
    ...
    // The held SHA gate is checked after recording the verdicts because that
    // GitHub round-trip gives a concurrent PR sync time to reveal a newer
    // push. A stale round reviews the new head instead of approving code it
    // never saw. Keep the card on screen if GitHub refuses the label so the
    // user can retry and the app never reports an approval that did not land.
    if (!kept.length && !heldRoundMoved(job, held)) {
  ...
  bus.emit('job', publicJob(job));
  if (!kept.length) {
    // Nothing to fix closes the round, not the loop, when the branch moved
    // while the round was held: those commits were never reviewed, and QA
    // must not run on them on the strength of a round that predates them.
    // The commit gate is re-run the way any settle runs it, and the answer
    // says which it did: a review started (the healthy outcome, not a fix
    // that failed to start), or none could start yet, since a review needs
    // the session idle and unasked, and a hold outlives a restart that left
    // it interrupted. The gate runs again at the next settle or push.
...
// Complete on the queue card of a hand-started review of the user's own pull
// request. Nobody else is going to fix what it found, so the card is the same
// gate a review-loop round gets and this is the same send: every finding is
// ruled, the verdicts go on the pull request, and what was marked fix starts
// the durable Implement feedback session the PR board's own button starts,
// with a review loop armed for the push that follows. With nothing marked fix
// the pull request is marked code-approved and the queue holder is deleted
// after the verdicts are recorded.
export async function triageStandaloneReviewFindings(id, { verdicts, note, by = 'the user' } = {}) {
  ...
  // Not the user's pull request: there is nothing to rule on it from here, and
  // a caller that sends verdicts anyway (an older tab, a script) gets the
  // completion this card actually has rather than an approval nobody asked for.
...
// Complete on the queue card of a review of somebody else's pull request. Its
// author is who answers it, not the person who ran it. So there is nothing to
// rule on here: what the
// review found is already on the pull request, in its threads and its findings
// block, and the card is the reviewer's own reminder that they have not read
// it yet. Completing takes it off the queue and takes the queue holder with
// it; nothing is written on the pull request, nothing is approved and no fix
// session starts. Whatever the reviewer had to say went on the threads
// themselves, one reply at a time (replyToReviewFinding).
// The round is released before the awaits below, so a second Complete (two
// tabs, a double click) finds nothing waiting instead of deleting twice.
...
// Reply on one finding's thread from its card: what the reviewer has to say
// goes under the comment the finding was raised in, where its author reads it,
// rather than into a comment of the dashboard's own. Nothing is recorded as a
// verdict and the card keeps the finding: a reply is a remark, not a ruling.
...
// Delete on one finding of a hand-started review: it leaves the review itself,
// not only this card. Its inline comment goes from the pull request and the
// review stops declaring it, so the author never reads a finding the reviewer
// withdrew and no later errand offers it again. Nothing is recorded as a
// verdict: a withdrawn finding is not a dismissed one, it is one the review
...
// The comments typed on a held loop round, before it is completed: the reason
// beside each finding, the verdict picked so far and the round's note. Saved
// onto the round itself, so a reload or another tab comes back to them, and
// posted on the pull request as one comment that every save rewrites
// (postTriageNotes), so what the person wrote is on the record even if the
// round is completed elsewhere or never. Nothing is ruled here: a verdict
// saved is a draft, and the loop goes on holding. The comment is best-effort
// on top of the record: a GitHub refusal comes back as a warning, with the
// drafts kept all the same.
/**
 * @param {string} id
 * @param {{ verdicts?: any[], note?: string, by?: string }} [opts]
 */
export async function saveReviewFindingsDrafts(id, { verdicts, note, by = 'the user' } = {}) {
  const job = jobs.get(id);
  if (!job || job.kind !== 'devchat') throw new Error('Session not found');
  // Every card that takes verdicts: a review-loop round, and a hand-started
  // review of the user's own pull request. A review of somebody else's is not
  // triaged here (its card replies on the threads instead), so it has no
  // verdicts and no comments to keep.
  const held =
  ...
  // A hand-started review's record is kept only to hold its card, and it is
  // closed the moment the review published: being retired says nothing about
  // whether its findings still wait.
...
// A completed round has its verdicts on the pull request in the "Review
// triage" comment, so the "so far" notes comment its saves wrote comes off;
// a refusal is nothing to fail the completion over.
...
// The developer UI accepts either source of queue cards. A hand-started
// review's card is completed the way its own card is drawn: with verdicts when
// it reviewed the user's own pull request, and with nothing to rule when it
// reviewed somebody else's. The agent endpoint remains loop-only, so an
// orchestrator cannot accidentally rule on a review the user started by hand.
...
// The last gates and the fix session behind them, for the findings a round
// is implementing: what the triage kept. Answers whether a fix session was
// started.
//
// The round cap stands here, on the round about to open the next one: the
// final round still reviews, is still held and ruled on, and its verdicts are
// still recorded on the pull request (recordTriage ran before this); what it
// does not do is start the fix session whose push would be reviewed as round
// max+1. Then the stall gate, the backstop for what the cap cannot see. Two
// rounds that hand back exactly the same findings mean the fix session
// between them did not move the review: implementing them again pushes
// another commit, which opens the sha gate, which starts the same round over:
// a ping-pong that holds a pooled session and spends tokens on a session
// nobody is watching. The loop stops instead and says why. What the round
// found is not lost: it is on the pull request either way.
...
// its later rounds so a retry away from an exhausted account does not have to
// be repeated every round. A retry that names a provider_id moves the whole
// loop onto it (loopSessionRuntime: the fix sessions and the QA run too); one
// that names only a model or an effort is about the review it re-runs, and is
// kept to the reviews (loop.reviewRuntime).
//
...
export async function retryLoopRound(id, { providerId, model, effort } = {}) {
  ...
  if (providerId != null || model != null || effort != null) {
    // What the call left out is filled in from what the part it moves already
    // runs on. For a retry that names only a model or an effort that is this
    // loop's reviews, not the session: the failure notice invites a retry
    // naming one of the reviewer's models, so defaulting the provider to the
    // session's would drop the model asked for and pin every later round to
    // the session's provider. A retry that names a provider moves the whole
    // loop, and what it settles goes to the fix sessions and the QA run, so it
    // keeps the model and effort the session's own work was started with
    // (ownRuntime): the reviewer's tier is nobody's answer for code-writing
    // turns, and an earlier retry's override is no answer either — moving back
    // across families would carry a model the named provider cannot run, which
    // resolveRuntime then swaps for that provider's default, so the call that
    // names the session's own row would be the one thing unable to put it back.
    const choice = loopReviewChoice(job);
    ...
    // A model the chosen provider does not run is swapped for that provider's
    // default (resolveRuntime), which is the wrong answer to a retry: a round
    // moved off an exhausted model would come back on the same one, and the
    // caller would never know. Say so instead, so the next retry can name a
    // model that provider has.
    ...
    if (providerId != null) {
      // A provider the caller named is the loop's, not the review's alone: it
      // is how an orchestrator moves a task off an account that is out of
      // quota, and the fix sessions and the QA run that continue that task go
      // with it, as they always have. It also replaces anything an earlier
      // partial retry had settled the reviews on, which named no provider and
      // so cannot outrank one that does.
      ...
      if (reviewer && resolved.provider.id === reviewer.provider.id) {
        // Naming the reviewer this loop gave up is the operator saying it is
        // repaired — the account rolled over, the CLI is installed now — so
        // take the flag off with it. Left on, loopReviewChoice would drop the
        // override it was just handed and review on the session's provider
        // while the fix sessions moved onto the provider the review refused.
        loop.reviewerFailed = false;
        // And the parts the call left out are the setting's answer again, not
        // the session's: filled in from what this moves (the fix sessions and
        // the QA run), they would review on the model that wrote the code, and
        // outrank ⌕ Code review for the rest of the loop since an override is
        // read ahead of the setting. So the reviews get the reviewer's own
        // runtime, with only what the caller actually named over it.
        loop.reviewRuntime = {
      ...
      // Only a model or an effort: what the call left out was filled in from
      // what the reviews run on, which may be the project's reviewer, and
      // writing that into the loop-wide override would move the fix sessions
      // and the QA run onto a reviewer this retry never pointed them at.
      loop.reviewRuntime = settled;
      // And whether that provider came out of the project's setting, so
      // repointing ⌕ Code review at another one moves these reviews with it
      // rather than leaving them frozen on a row nothing names (loopReviewChoice).
  ...
  // The model the round will actually open on rather than the one the loop has
  // pinned: loopReviewChoice can pass a pinned review override over (its
  // provider was filled in from ⌕ Code review and Settings names another one
  // now), and this line is the operator's only word on the runtime. A round
  // that is simply the session's own is not worth naming, as before.
...
function notifyLoopReviewFailed(review, { fromProvider = false } = {}) {
  ...
  failLoopRound(parent, why);
  // A round that died *on the provider* while riding the project's reviewer is
  // that reviewer failing, not the session's: an account with nothing left in
  // its window, a login that expired. The next push would pick the same one
  // and die the same way, so give the reviewer up for this loop and review
  // that push on the session's own provider rather than reviewing nothing at
  // all. Everything else that ends a review session says nothing about which
  // reviewer it was riding — a server restart mid-round, a clone that failed,
  // one of the project's setup steps, a turn the time limit cut off or one the
  // CLI itself reported failed while exiting cleanly — and the round is
  // retried on the reviewer as it always was.
  ...
  save(parent);
  // The orchestrator is the one party that acts on this, and the action it is
  // told to take moves the loop's runtime — its fix sessions and its QA run
  // with it (retryLoopRound). So it is told to move only when the runtime is
  // what let the round down. A round the process dying, the workspace clone,
  // one of the project's setup steps, the turn's own time limit or the CLI's
  // own report of a failed turn ended says nothing about the provider it was
  // riding, and moving off the reviewer this project was set up with for one
  // of those is the very thing the gate above exists to prevent.
  notifyParentLoop(
    parent,
    fromProvider
      ? `the review loop's code review could not run (${why}). This is the provider failing, not a review that found nothing: round ${parent.reviewLoop.rounds} approved nothing. Retry it with retry_review — on a different provider_id (another service, since one provider_id's accounts are already balanced by quota) or model if this one is out of quota — since a worker with nothing left to push will never start the round itself.${loopWideUndoneNote(parent, undone)}`
      : `the review loop's code review could not run (${why}). The round was interrupted rather than turned away by the provider — a restarted dashboard, the workspace clone or one of the project's setup steps, a turn the time limit cut off or one the CLI reported failed — so round ${parent.reviewLoop.rounds} approved nothing. Retry it as it was with retry_review, naming no provider_id or model: nothing here says the runtime it ran on is the problem, and naming one would move this loop's reviews, its fix sessions and its QA run off it — since a worker with nothing left to push will never start the round itself.`,
  );
}
...
async function maybeStartLoopQa(job) {
  ...
  // loop's fix sessions do (loopSessionRuntime). The loop's reviews are the one
  // part that no longer does: they follow the project's reviewer
  // (loopReviewChoice).
  const project = getProject(job.repo);
      ...
      ? {
          ...
          triage: job.reviewLoop.triage
            ? {
                ...
                // The branch moved since the round was reviewed: some of these
                // may already be fixed, and a send that fixes nothing reviews
                // the new commits rather than converging.
...
// association forever. `details` says whether that sync is a forced one (see
// syncDevPr), which a forced caller queued behind it can join.
const prSyncInflight = new Map(); // session id -> { promise, details } of the sync in flight

// The pull request's reviews, commits, linked issues and the head commit's
// check runs, read in one GraphQL round trip. Each list is written onto
// `status` only when the answer carries it, so a query that fails keeps
// whatever the last sync mirrored: best-effort, like the rest of the sync (a
// list the token may never read is recorded as unknown instead). The checks
// are read off the commit the REST answer named as the head, not off the pull
// request's commit list: that list stops at 250 commits, and a push landing
// between the two reads would otherwise pair the new head's checks with the
// old head's sha. A check run's `url` is its page on GitHub, the link the
// panel has always given it; checkRunsOf falls back to it for want of a
// detailsUrl, which this query leaves out.
...
// Answers whether every list was refreshed. One that failed or came back
// missing keeps the last sync's copy, and the caller must not count it as a
// read: the skip would then pin that copy (the old head's checks, after a push
// whose rollup read timed out) until the read next expires.
async function readPrDetails(cfg, repo, number, status) {
  ...
  // GraphQL reports a field it could not resolve by nulling it and naming its
  // path in the errors, so a null under a failed path is "unknown", not "none".
  // Only an error on the list itself (or on what holds it) fails the list: one
  // on a single node inside it (a review's author, one check context) nulls
  // that node alone, which is dropped and the rest kept, rather than throwing
  // the whole list away (and, if the error is persistent, querying again on
  // every sync).
  const errorsOn = (list) =>
    errors.filter(
      (er) =>
  ...
  // A token that may never read a list (a fine-grained token without Issues
  // read, on the linked issues) never will: nothing is known of it, and the
  // read still counts, or every sync would query again for it.
  ...
  let complete = true;
  // The three lists on the pull request itself, one rule: refreshed when the
  // answer carries the list, recorded as unknown when the token may never read
  // it, and otherwise left as the last sync mirrored it, which is a read that
  // fell short.
  const lists = [
  ...
  // The head commit's checks. A commit with no rollup has no checks at all;
  // that is an answer too (total 0), not a failed read. A rollup nulled by an
  // error (a rollup that timed out) is not, and keeps the checks the last sync
  // mirrored; one the token may not read at all is recorded as unreadable.
  ...
  if (headCommit && 'statusCheckRollup' in headCommit && !failed(checksPath)) {
    ...
    // A token that may not read checks at all (a fine-grained token without
    // Checks read) never will: no checks known, and remembered as such, so the
    // skip does not wait on a rollup that cannot settle.
    status.checks = null;
...
// The longest the details read is skipped on a pull request nothing has
// nudged: what its tag does not cover (a linked issue's state, a CI that
// registers late, a GraphQL read that answered from just before an event) is
...
// The checks that make a CI verdict here: the check runs. A commit status is
// shown on the panel with them, but many are advisory (a coverage delta, a
// preview deploy) and nothing says which ones branch protection requires, so
// one failing must not tell an orchestrator not to merge, nor one stuck
// pending keep the pull request polled. The same set the REST check-runs read
// this replaced counted.
...
// Whether the check runs on the same head are not the ones the last sync
// mirrored. By run identity where both reads carry it (a re-run of a failed
// job is a new run with a new id and completion time, whatever it concludes),
// by the counts where one side predates the ids (a record written before the
...
// Whether a CI verdict is still to come on the mirrored head: check runs still
// going, or a head too fresh for its CI to have registered (none yet, or none
// read yet). Repository hooks hear a suite finish (check_suite completed), not
// one start, so such a head is what keeps the minute cadence and the details
// read going, and a sync that saw it is one that watched the run.
...
// `details` is a webhook's nudge: something moved on the pull request just
// now, so the GraphQL details are read whatever its ETag says (a check suite
// re-run on the same head moves the checks without touching the pull request
// resource), and a sync already in flight is not enough.
async function syncDevPr(job, number = null, { spotted = false, strict = false, details = false } = {}) {
  ...
  while (prior) {
    ...
    // mirror it (the common case is a turn-end refresh with no PR yet). A
    // nudge does too: the sync it waited on may have read GitHub before the
    // event, and on a hooked repository the next tick is a long way off.
    if (!details && (number == null || (job.prStatus && job.prStatus.number === number)))
      return job.prStatus || null;
    // Several callers can be queued behind the same sync (a burst of check
    // events, a push's synchronize beside them). The first one to wake starts
    // the pass; the rest must not each start another from the same `before`,
    // each with its own reads and its own notifyParentChecks. A forced read
    // started since began after all their events, so a forced caller joins
    // it; any other sync is waited out like the first one.
...
async function syncDevPrNow(job, number = null, { spotted = false, strict = false, details = false } = {}) {
  ...
  try {
    ...
    const status = {
      ...
      url: pr.html_url,
      title: pr.title,
      // Who opened it: what tells a review of the user's own work from a
      // review of somebody else's (pullRequestAuthor).
      author: (pr.user && pr.user.login) || null,
      ...
      // A CI verdict this session owes its orchestrator: the head moved under
      // it (a push it saw land), and nothing has been handed over since. Kept
      // across a details read that fell short, so the verdict is not lost with
      // it (see the notifyParentChecks call below).
      ...
      // The tag this sync's REST read saw, for the next sync to tell whether
      // the pull request moved in between (see the details read below).
      etag: pres.etag || null,
      syncedAt: now(),
    };
    // Everything else the panel shows (reviews, commits, linked issues, CI)
    // comes from one GraphQL query, on GraphQL's own budget, and only when
    // there can be something new to read: a webhook nudged the session, the
    // head moved (or this is the first sync), the pull request answered with a
    // tag other than the one this session's last sync saw (the tick on a
    // repository with no hook), or a CI verdict is still to come
    // (checksAwaited). This session's own last tag, not the 304: the ETag
    // cache is shared by every caller and session, so a 304 only says nothing
    // changed since *somebody* last read it. Anything else waits for the read
    // to be DETAILS_MAX_AGE_MS old, which a read that fell short counts as.
    ...
    if (details || moved || checksAwaited(before) || olderThan(before.detailsReadAt, DETAILS_MAX_AGE_MS)) {
      ...
      // A read made under a tag this session had not seen before is not the
      // last word: GraphQL can answer from just before the change the tag
      // announced (a review submitted, a webhook nudge later). It counts as
      // read, and the skip starts, only once a sync has read under a tag an
      // earlier sync already saw, so one more read follows the first (the tick
      // keeps the minute for it, see syncDevPrs).
      // No tag at all (GitHub answering without one) leaves nothing to confirm
      // against, and the read counts as it is.
    ...
    // A round held for a pull request that is merged or closed has nothing to
    // send its verdicts to; triageLoopFindings would take them and start
    // nothing. And a head that moved under the hold is a push the round never
    // saw, said on the card.
    if (status.state !== 'open') {
      ...
      // The loop's gates read the clone's own remote-tracking ref
      // (loopHeadSha), which a push made anywhere but this clone (by hand,
      // from another checkout) never moves, whether a round is held, under
      // review or not started yet. So teach the clone about a head it has
      // never seen the moment the sync sees it; the probe falls back to the
      // mirrored head until the fetch has landed, so a fetch that fails
      // (retried on the next head move) leaves the gates reading the right
      // commit all the same. The clone's own push needs no fetch.
    ...
    // this session watched move: either the head moved under it, which is a
    // push it saw land (awaitingVerdict, kept until the verdict is handed
    // over), or a read on this head had already been mirrored and it was
    // awaited (running, or a fresh head whose CI had not registered yet) or
    // the runs are not the ones that read saw. That last one is a run whose
    // start no sync saw: a re-run of a failed job, or a suite that registered
    // after the first one settled, since repository hooks hear a suite finish
    // and not start. The first sync of a pull request says nothing, and
    // neither does the first read that succeeds on it — a fresh attach and the
    // first tick after a restart both find a finished run that nobody is
    // waiting on any more, whether that first read went through or not.
    // Another head's checks are dropped at the move, so checks on the record
    // are a read that succeeded on this very head.
...
// one that lands while a lookup is in flight (a pull_request/opened delivery
// racing the tick, whose list request went out before the PR existed) asks
// again the moment that lookup ends empty-handed.
...
// The repositories whose GitHub webhook is live, and when a signed delivery
// from each last arrived (lib/webhooks.js stamps every one). A session there
// hears about every push, review, comment and CI verdict as it happens, so the
// timer only has to cover a lost delivery. Live means delivering, not merely
// installed: a hook GitHub accepted can still reach nothing (a tunnel that is
// down, a proxy that rejects the path, a hook somebody disabled after boot),
// so a repository that has gone an hour without a delivery is back on the
// minute cadence, which the conditional GETs keep nearly free anyway.
// A delivery proves the hook reaches this install, not that it carries every
// event the long cadence counts on: an older hook this boot could not bring up
// to date (a PATCH refused or failed) still delivers pull_request but not
// `status`. So only a repository whose hook this process installed, or found,
// with the full event list (lib/webhooks.js installRepoWebhooks) gets it.
...
export function repoHasWebhook(repo) {
  ...
  // Said once per quiet spell, so a hook that stopped delivering is not a
  // silent change of cadence.
  if (!hook.quietSaid) {
    hook.quietSaid = true;
    console.log(
...
// already knows its PR, while a turn is in flight or the PR still has news
...
function syncDevPrs() {
  for (const job of jobs.values()) {
    ...
    // Only a pull request that can still move is worth asking about: one that
    // is open, one whose CI is still running, or one under a turn in flight.
    // An idle session on a merged or closed pull request used to be polled
    // once a minute for the rest of its life, and with a few dozen of those
    // the tick alone spent the whole hourly budget.
    // A CI verdict still to come keeps a merged or closed one watched only for
    // a while after its head was first seen (PENDING_WATCH_MS).
    ...
    // The rest wait a minute, or, on a repository whose webhook is live and
    // already nudges the session on every event, a long safety net for the
    // delivery that never arrived.
    const age = Date.now() - Date.parse(job.prStatus.syncedAt || 0);
    // A CI verdict still to come keeps the minute too: the hook only hears
    // whole suites finish (check_suite), not one start, and the panel lists
    // every run as it completes.
    // So does a details read still to be confirmed (detailsReadAt unset): the
    // read that follows a change is checked by one more, a minute later.
...
// milliseconds, and a sync that fails is one the tick will retry anyway. Every
// one is a nudge (syncDevPr's `details`): the details are read whatever the
// pull request's ETag says, since CI moves without touching it, and a sync
// already in flight gets a follow-up pass rather than standing in for this one.
...
async function prepareDevWorkspace(job, { preserve = false } = {}) {
  ...
  // Reopening a conversation must not reset commits or clean its unfinished
  // files. A recycled slot still takes the normal branch preparation below.
  if (preserve && job.branch && slotBranch(dir) === job.branch) {
  ...
  const plan = workspaceBranchPlan(job, base);
  const start = workspaceStartBranch(job);
  // GitHub exposes every pull request under the base repository's
  // refs/pull/<n>/head, including PRs opened from forks. A board preview uses
  // that immutable address instead of guessing that the head branch exists on
  // origin; it also means the checkout is exactly the PR the server verified.
...
// guessing. One line per group, exactly as the operator's own picker shows
// them: several logins to one service are one choice, since naming any of
// them starts the worker on whichever has the most quota left. Resolved fresh
// each turn: entries and their model lists are edited in Settings while
// sessions run.
...
${workerRuntimeBriefing(job)}${budgetBriefing(job)}${toolingBriefing(job)}${projectInstructions(job)}${memoryProtocol(job)}${sshProtocol()}${ASK_PROTOCOL}`;
}
...
${zeusRolesBriefing(job) || workerRuntimeBriefing(job)}${budgetBriefing(job)}${toolingBriefing(job)}${zeusEpicTemplate(job)}${memoryProtocol(job)}${sshProtocol()}${ASK_PROTOCOL}`;
}
...
async function probeContextUsage(job, prov, chat, model) {
  ...
  const run = (async () => {
    try {
      ...
      else if (prov.binary === 'codex') codexContextFromRollout(job, prov, chat.sessionId, model);
    } catch {
...
function codexContextFromRollout(
  ...
  if (!sessionId) return;
  const file = rolloutFile || findCodexRollout(path.join(codexHomeDir(prov), 'sessions'), sessionId);
  if (!file) return;
...
function runDevTurn(job, prompt, { review = false, step = null } = {}) {
  ...
  job.turnCanceled = false;
  // Whether the limit below is what killed this turn. A kill leaves no exit
  // code, so the turn rejects the way a crashed CLI does, and only this says
  // the two apart (see the catch in startDevSession).
  job.turnTimedOut = false;
  // And whether the CLI reported the turn failed while exiting cleanly, which
  // reads as a failed session the same way a non-zero exit does, and is the
  // provider's content rather than the provider.
  ...
  // Cost and time go onto the session, and into the ledger, as what the turn
  // spent since the last time this ran. A claude process taking messages live
  // runs it on every answer: one process can live for hours, and a restart
  // before it exits would lose every answer it gave. Close takes the rest,
  // which for every other turn is all of it.
  const recorded = { costUsd: 0, durationMs: 0, inputTokens: 0, outputTokens: 0, rows: 0 };
  const takeUsage = () => {
    const part = {};
    for (const k of ['costUsd', 'durationMs', 'inputTokens', 'outputTokens']) {
      part[k] = turn[k] != null ? turn[k] - recorded[k] : null;
    }
  ...
  return new Promise((resolve, reject) => {
    ...
    child.stdin.on('error', () => {});
    // The limit runs from the latest message, not from the spawn: a claude
    // process can take many of them while its background work is out, and
    // each is owed the same time a turn of its own would have had.
    const armLimit = () => {
      if (!limitMin) return;
    ...
    // A stream-json CLI keeps reading stdin for the whole turn, so messages go
    // in as they are sent; settleInput closes it once every message has been
    // answered with nothing left in the background, which is what lets the
    // process exit.
    let input = null;
    // The sub-agents and Monitors the CLI is still waiting on.
    let backgroundTasks = [];
    // What the CLI was told and has not answered yet, oldest first. The echo
    // (an ack) says it was read, and the next result answers every message
    // read by then: one sent mid-answer is folded into that answer. The
    // result's own queued_turn_count cannot stand in for this count: the
    // 2.1.281 CLI reports 0 even with messages it has read waiting behind it.
    // Every answer opens with an init, counted here: a wake-up that ends
    // answers the wake-up entries added before its init, and no message. The
    // answer under way answers one once nothing it waits for can wake the
    // CLI any more: the agent stopped those tasks itself, or their
    // notifications were read into that answer. A message and a notification
    // both waiting between answers get one each: probed, the 2.1.281 CLI
    // answered them apart, in the order they came.
    let inits = 0;
    let wakeUp = false;
    const answered = () => {
      const before = input.pending;
      const wake = wakeUp;
      wakeUp = false;
      const woke = (p) => p.wake && ((wake && p.after < inits) || !p.tasks.length);
      const left = before.filter((p) => !woke(p));
      if (wake) input.pending = left.filter((p) => !p.acked);
        ...
        // closes early and the CLI still answers what it was sent before
        // exiting. A wake-up has an entry of its own (see 'background' below).
        if (unread.length === left.length) unread.shift();
        input.pending = unread;
      }
      // A new Zeus brief read into an answer already under way starts its
      // round once that answer, which may still be spawning the old round's
      // models, is over.
      for (const p of before) if (p.entry && !input.pending.includes(p)) deliverFusionBrief(job, p.entry);
    };
    // Messages waiting when the CLI starts its next answer come back as one
    // echo, joined by newlines, so the newest one it carries is its last
    // part. The CLI reads them in order, so an echo acks that message and
    // every one sent before it, including one whose echo is not its own text
    // (a prompt command comes back expanded). An echo that is exactly some
    // of them wins over one that only ends with one: a short reply ("yes")
    // can be a line of another message. An echo that carries none of them is
    // the oldest one's.
    const ack = (text, opens) => {
      ...
      // A new Zeus brief that opens an answer of its own starts its round now:
      // the old round's answer is over. One folded into an answer under way
      // waits for its result (see answered).
      if (opens) for (const p of read) if (p.entry) deliverFusionBrief(job, p.entry);
      // What the user sent is the answer to a question the agent asked
      // meanwhile only now that the CLI has read it (see markDelivered).
      if (read.some((p) => p.entry) && (job.awaitingAnswer || job.lastTool)) {
        job.awaitingAnswer = false;
        job.lastTool = null;
        save(job);
        bus.emit('job', publicJob(job));
    ...
    // Nothing from the CLI for this long with stdin open, every message
    // answered and no background work out means something is off, most
    // likely a wake-up that never came. Closing stdin costs nothing if it is
    // still working: the CLI finishes what it was doing, answers, and exits;
    // new messages just queue meanwhile. A message still waiting on its
    // answer keeps stdin open however long it takes: a foreground build or
    // test suite prints nothing to the stream while it runs. So does a
    // background task: a sub-agent's long build, or a Monitor waiting on its
    // event, is silent too. The CLI's own task list keeps that count honest,
    // the process cannot exit before that work ends anyway, and the time
    // limit still bounds it.
    // One timer for the whole turn: output only moves lastOutputAt, and the
    // timer, when it fires early, waits out whatever quiet time is left.
    ...
    // Only a turn on the session's own conversation takes messages live. A
    // step on another provider (a test run on its own claude account) holds a
    // conversation of its own: what the user sends meanwhile is meant for the
    // session's agent, so it queues for the drain, which runs it there.
    // Compared against the resolved row, not job.providerId: a session stored
    // from before providers moved into the database has only a slug.
    if (build.promptVia === 'stream-json' && String(prov.id) !== String(getProviderForJob(job)?.id)) {
      ...
      armQuiet(quietMs);
      // Whatever was queued before the turn could take input goes in now, in
      // order, rather than waiting for this whole process to exit.
    ...
    // A live answer's footer goes out once its ledger row has landed, as at
    // close (the result stream estimates from it). The lines after it wait
    // too, so the transcript keeps the CLI's order.
    let held = null;
    ...
    child.stdout.on('data', (chunk) => {
        ...
        try {
          for (const e of parser.feed(JSON.parse(line))) {
            ...
            if (e.kind === 'background') {
              // The last task reporting done wakes the CLI for one more
              // answer, which can start new background work of its own, so
              // stdin stays open until that answer is in. Only a wake-up
              // that opens after this answers it: the result of an answer
              // already under way when the task ended leaves it waiting.
              // Its entry holds no message: the quiet timer may still close
              // stdin if the answer never comes. It names the tasks that
              // ended, since one the agent stopped, or whose notification the
              // answer under way read in, wakes nobody (task_settled).
      ...
      if (prov.binary === 'codex' && turn.sessionId && Date.now() - lastUsagePush > 2000) {
        ...
        try {
          codexRolloutFile ||= findCodexRollout(path.join(codexHomeDir(prov), 'sessions'), turn.sessionId);
          if (codexRolloutFile)
            codexContextFromRollout(job, prov, turn.sessionId, model, false, codexRolloutFile);
        } catch {
          /* best effort */
        }
      }
    ...
    child.on('close', async (code) => {
      ...
      save(job);
      emitUsage(job);
      // The live result stream estimates from this ledger row. Land it before
      // publishing the footer so the SSE endpoint cannot race the write.
      if (part) await recordTurnUsage(job, part, prov, model);
      for (const data of resultEvents) pushEvent(job, 'result', data);
      // Messages the CLI was handed live and never answered die with it. On a
      // Stop they go back to the head of the queue, where they would have
      // waited before, so the drain that follows acts on them next: "stop, do
      // this instead" is exactly what gets sent mid-run. A crash, a timeout
      // or a failed stream rejects the turn, and no drain follows that: kept
      // in the queue, they would run unannounced whenever the session next
      // reopens, long after they were sent. So they are dropped, and the
      // transcript says which. A close drops them along with the rest of the
      // queue.
      ...
      // A clean exit with a failed stream is the CLI reporting on its own work
      // — opencode's normal way of ending a turn on a tool error or a refusal
      // — and not the account behind it turning the turn away, which is what
      // the loop reads to give a reviewer up (see the catch in startDevSession).
      if (code === 0) job.turnStreamFailed = true;
      reject(
        new Error(
