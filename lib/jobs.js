...
import { publicAppUrl } from './tunnel.js';
...
export function publicJob(job, ledgerUsage = null) {
  ...
  const { events, seq, proc, timeout, turnCanceled, turnTimedOut, turnStreamFailed, serveProc, ...meta } =
    job;
...
export async function startPullRequestPreview({ provider, model, effort, repo, branch, prNumber, title }) {
  ...
  for (const old of stale) {
    ...
    // Somebody may have typed into it while it was still preparing.
    if (!pristinePreview(old)) continue;
  ...
  let served;
  try {
    served = await startDevServe(job.id);
  } finally {
    // A message typed while the workspace was being prepared was queued like
    // any mid-turn message, but a preview has no opening turn to drain it at
    // the end of. It goes now, whether or not the app came up.
    sendQueuedMessages(job);
  }
...
// A preview nobody has said anything in yet. `previewKept` is set by the first
// message, before the provider has answered (hasProviderChat only turns true
// once a turn finishes), so neither a turn still in flight nor one still queued
// behind the preparation reads as an abandoned environment.
function pristinePreview(job) {
  return !!job.preview && !job.previewKept && !hasProviderChat(job);
}
...
${zeusRolesBriefing(job)}\n</workspace-context>`;
  ...
  // A ▶ Run preview somebody chats in is theirs from the first word, not an
  // environment the expiry sweep or the next ▶ Run may throw away.
  if (job.preview && !job.previewKept) {
    job.previewKept = true;
    save(job);
  }
  ...
  deliverDevMessage(job, { prompt, shown, files, newFusionBrief });
  return publicJob(job);
}

// Send whatever is queued on a session that has come to rest without a turn
// of its own to drain it at the end of (a ▶ Run preview, once prepared): the
// first message starts a turn, and that turn's drain takes the rest.
function sendQueuedMessages(job) {
  const queue = devQueues.get(job.id);
  if (!queue?.length || job.status !== 'idle') return;
  const next = queue.shift();
  if (!queue.length) devQueues.delete(job.id);
  deliverDevMessage(job, next);
}

// Run one message as a turn on a session that is not mid-turn, reopening its
// workspace first when it has none, then drain anything queued behind it.
function deliverDevMessage(job, { prompt, shown, files, newFusionBrief }) {
  if (newFusionBrief) {
    job.zeusProposalPrompt = null;
    save(job);
  }
...
export async function startDevServe(id) {
  ...
  if (job.serveProc && job.serveProc.exitCode === null) {
    return { url: await servedUrl(job, job.appPort) };
  }
...
// that fails the app is still up, so the loopback URL goes out with the reason
// in the log rather than the whole Run failing.
async function servedUrl(job, port) {
  const local = `http://127.0.0.1:${port}`;
  try {
    return (await publicAppUrl(port)) || local;
  } catch (e) {
    pushEvent(job, 'stderr', {
      text: `Could not publish port ${port} through the Cloudflare tunnel: ${e.message}`,
    });
    return local;
  }
}
...
async function startDevServeProc(job) {
  ...
  const published = servedUrl(job, job.appPort);
  ...
  pushEvent(job, 'info', {
    text: `Serving the workspace at ${local} (${recipe.command}, session database ${job.dbHost ? `${job.dbHost}:${job.dbPort}` : 'n/a'})`,
  });
  save(job);
  ...
  const url = await published;
  if (url !== local) pushEvent(job, 'info', { text: `Published through the tunnel at ${url}` });
  return { url };
}
