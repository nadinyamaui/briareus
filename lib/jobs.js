...
export function publicJob(job, ledgerUsage = null) {
  ...
  meta.liveInput = liveInputs.has(job.id);
...
// background. A message sent meanwhile goes straight into that CLI, mid-turn
// or while its sub-agents and Monitors are still out, instead of waiting in
// devQueues for all of it to finish.
const liveInputs = new Map(); // job id -> { pending, send(entry) }
// The same input for the turn's whole life, also once it stops taking
// messages: what it was handed and has not answered still counts against
// MAX_QUEUED, since a killed turn puts it back in the queue.
const turnInputs = new Map(); // job id -> { pending, send(entry) }
...
// A turn that failed leaves the session idle, and nothing drains the queue
// then: what it holds (the messages the dead turn was handed and never
// answered, and any sent behind them) would sit there as queued with no turn
// coming, hold back the review loop, and run after the next message instead
// of before it. So it is dropped, and the transcript says what was.
function dropQueueAfterFailure(job) {
  const left = devQueues.get(job.id) || [];
  devQueues.delete(job.id);
  if (!left.length) return;
  const quoted = left.map((entry) => `"${entry.shown.slice(0, 200)}"`).join(', ');
  pushEvent(job, 'info', {
    text: `The failed turn never answered ${left.length} message(s); they were not sent again, so send what still matters once more: ${quoted}`,
  });
  bus.emit('job', publicJob(job));
}
...
// is put back in the queue when it closes (see runDevTurn).
function detachLiveInput(job) {
  if (!liveInputs.delete(job.id)) return;
  bus.emit('job', publicJob(job));
}
...
${zeusRolesBriefing(job)}\n</workspace-context>`;
  ...
  if (ACTIVE.includes(job.status)) {
    const input = liveInputs.get(job.id);
    if (input) {
      assertQueueRoom(job);
...
function runDevTurn(job, prompt, { review = false, step = null } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // read by then: one sent mid-answer is folded into that answer.
    const answered = () => {
      const unread = input.pending.filter((p) => !p.acked);
      // A local command (/context) answers without an echo. So does a
      // finished sub-agent waking the CLI, which can take this for a message
      // written just before it: at worst stdin closes early and the CLI still
      // answers what it was sent before exiting.
      if (unread.length === input.pending.length) unread.shift();
      input.pending = unread;
    };
    // Nothing from the CLI for this long with stdin open means something is
    // off, most likely background work it never reported finishing. Closing
    // stdin costs nothing if it is still working: the CLI finishes what it
    // was doing, answers, and exits; new messages just queue meanwhile.
    ...
    const closeInput = () => {
      clearTimeout(quiet);
      liveInputs.delete(job.id);
      child.stdin.end();
      bus.emit('job', publicJob(job));
    };
    const armQuiet = (ms) => {
      quiet = setTimeout(() => {
        if (liveInputs.get(job.id) !== input) return;
    ...
    if (build.promptVia === 'stream-json' && String(prov.id) !== String(job.providerId)) {
      child.stdin.end(claudeInputMessage(fullPrompt));
    } else if (build.promptVia === 'stream-json') {
      input = {
        pending: [{ entry: null, acked: false }],
        send(entry) {
          input.pending.push({ entry, acked: false });
      ...
      while ((idx = buffer.indexOf('\n')) !== -1) {
        ...
        try {
          for (const e of parser.feed(JSON.parse(line))) {
            ...
            if (e.kind === 'ack') {
              const read = input?.pending.find((p) => !p.acked);
              if (read) read.acked = true;
              continue;
            }
    ...
    const dropInput = () => {
      clearTimeout(quiet);
      if (liveInputs.get(job.id) === input) liveInputs.delete(job.id);
    };
    const dropTurnInput = () => {
    ...
    child.on('close', async (code) => {
      ...
      // Messages the CLI was handed live and never answered die with it when
      // it is stopped, runs out of time or crashes. They go back to the head
      // of the queue, where they would have waited before, so the next turn
      // acts on them: "stop, do this instead" is exactly what gets sent
      // mid-run. A close drops them along with the rest of the queue, and so
      // does a failure that leaves the session idle (dropQueueAfterFailure).
      const unanswered = input ? input.pending.filter((p) => p.entry).map((p) => p.entry) : [];
      if (
        unanswered.length &&
        (job.turnCanceled || code !== 0 || streamFailed) &&
        !job.closing &&
        job.status !== 'closed'
      ) {
