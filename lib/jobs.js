...
import { githubRest, githubGraphql, upsertPrComment, addPullRequestLabel } from './github.js';
import {
  saveJob,
  saveJobEvents,
  loadJobs,
...
export async function triageLoopFindings(id, { verdicts, note, by = 'the orchestrator' } = {}) {
  ...
  try {
    ...
    if (!kept.length && !heldRoundMoved(job, held)) {
      await addPullRequestLabel(getConfig(), job.repo, held.prNumber, CODE_APPROVED_LABEL);
    }
  } finally {
