...
function startDevSession(job, prompt) {
  (async () => {
    try {
      ...
      // evidence about the provider (see notifyLoopReviewFailed).
      const fromProvider = job.status === 'running';
      if (job.status !== 'closed') {
...
function newReviewLoop() {
  return {
    ...
    // or none that can run" (loopReviewChoice).
    runtime: null,
...
function notifyLoopReviewFailed(review, { fromProvider = false } = {}) {
  ...
  // one of the project's setup steps — and the round is retried on the
  // reviewer as it always was.
  const gaveUp = fromProvider && !!parent.reviewLoop.reviewerRound;
  if (gaveUp) parent.reviewLoop.reviewerFailed = true;
