...
import { pickRunProfile, profileRun, runVars } from './runprofiles.js';
...
function devServeRecipe(job, profile) {
  ...
  // The profile's own lines are checked against its tenants when the profile
  ...
  const missing = commands
    .flatMap((c) => [...c.matchAll(/\{host:([\w-]+)\}/g)].map(([, t]) => t.toLowerCase()))
    .find((t) => !Object.prototype.hasOwnProperty.call(vars, `host:${t}`));
  if (missing) {
    return {
...
// through it the page) in `all`, once they are in. `all` never rejects: a
...
// What ▶ answers with: the tab to open. The other links follow on their own.
async function servedTab(job) {
  const { first } = servedLinks(job);
  return { url: (await first).url, profile: job.serveProfile || null };
}
...
async function startDevServeProc(job, profile, recipe = null, dir = job.workDir) {
  ...
  const published = servedLinks(job);
  ...
  published.all
    .then(({ links, live }) => {
      ...
      } else if (first.url !== local) {
        pushEvent(job, 'info', { text: `Published through the tunnel at ${first.url}`, links });
      }
    })
    .catch((e) => console.error('serve links error:', e.message));
  return { url: first.url, profile: profile ? profile.name : null };
}
