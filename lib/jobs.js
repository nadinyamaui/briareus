...
import { reviewsOf, checkRunsOf, checksSummary } from './prboard.js';
...
async function readPrDetails(cfg, repo, number, status) {
  ...
  const errorsOn = (field) => errors.filter((er) => Array.isArray(er?.path) && er.path.includes(field));
  const failed = (field) => errorsOn(field).length > 0;
  let complete = true;
  if (node.reviews?.nodes && !failed('reviews')) status.reviews = reviewsOf(node);
  else complete = false;
  // The commits, for the panel's collapsible list: first message line, sha
  // and a link each.
  if (node.commits?.nodes && !failed('commits')) {
    status.commitList = node.commits.nodes.map(({ commit }) => ({
      sha: commit.oid,
      message: String(commit.messageHeadline || '').slice(0, 140),
      url: commit.url,
    }));
  } else complete = false;
  // The issues this PR closes: its "Development" links on GitHub.
  if (node.closingIssuesReferences?.nodes && !failed('closingIssuesReferences')) {
    status.issues = node.closingIssuesReferences.nodes.map((i) => ({
      number: i.number,
      title: i.title,
  ...
  if (headCommit && 'statusCheckRollup' in headCommit && !failed('headCommit')) {
    status.checks = checksSummary(checkRunsOf(headCommit.statusCheckRollup));
    status.checksUnreadable = false;
  } else if (errorsOn('headCommit').some((er) => PERSISTENT_GRAPHQL_ERRORS.has(er?.type))) {
...
async function syncDevPrNow(job, number = null, { spotted = false, strict = false, details = false } = {}) {
  ...
  try {
    ...
    // not have registered when it was read (CHECKS_REGISTER_MS).
    const etag = pres.etag || null;
    ...
    if (details || !unchanged || !checksSettled) {
      if (await readPrDetails(cfg, job.repo, number, status)) status.detailsEtag = etag;
    }
    job.prStatus = status;
