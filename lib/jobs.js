...
function markDelivered(job, entry) {
...
function runDevTurn(job, prompt, { review = false, step = null } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // echo, joined by newlines. The CLI reads them in order, so an echo acks
    // the newest message it carries and every one sent before it, including
    // one whose echo is not its own text (a prompt command comes back
    // expanded). An echo that carries none of them is the oldest one's.
    const ack = (text) => {
      const unread = input.pending.filter((p) => !p.acked && !p.wake);
      let rest = typeof text === 'string' ? `\n${text}\n` : '';
      let upTo = 0;
      unread.forEach((p, i) => {
        const at = rest.indexOf(`\n${p.prompt}\n`);
        if (at === -1) return;
        upTo = i;
        rest = rest.slice(at + p.prompt.length + 1);
      });
      for (const p of unread.slice(0, upTo + 1)) p.acked = true;
    };
        ...
        try {
          for (const e of parser.feed(JSON.parse(line))) {
            ...
            if (e.kind === 'background') {
              ...
              // stdin stays open until that answer is in. It has no echo:
              // its init marks it read, so the result of an answer already
              // under way when the task ended leaves it waiting. Its entry
              // holds no message: the quiet timer may still close stdin if
              // the answer never comes.
              if (input && backgroundTasks.length && !e.tasks.length) {
                input.pending.push({ entry: null, acked: false, wake: true });
              }
              backgroundTasks = e.tasks;
              continue;
            }
            if (e.kind === 'init') {
              const wake = input && input.pending.find((p) => p.wake && !p.acked);
            ...
            if (e.kind === 'result' && input) {
              ...
              if (part) {
                ...
                save(job);
                emitUsage(job);
                recordTurnUsage(job, part, prov, model);
              }
            }
            const { kind, ...data } = e;
