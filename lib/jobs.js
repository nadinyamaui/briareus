...
function devServeRecipe(job, profile) {
  ...
  const database = sessionDatabaseName(job) || (job.local ? checkoutDatabaseName(job.workDir) : '');
...
function checkoutDatabaseName(dir) {
  ...
  const m = text.match(/^DB_DATABASE=(.*)$/m);
...
export async function startDevServe(id, { profile: wanted = null } = {}) {
  ...
  // Taken on the way in, so a click still waiting below on another start is
  // called off by a close as well as the start it waits on.
  const epoch = job.serveEpoch || 0;
  // A plain click during a switch means the profile being switched to, not the
  // one still recorded as served: resolving it to that one would undo the switch.
  const profile = pickRunProfile(getProject(job.repo), wanted, job.serveStartingProfile ?? job.serveProfile);
  ...
  // Hand every concurrent caller asking for the same profile the same
  // in-flight start instead; one asking for another waits for it to land.
  while (job.serveStarting) {
    if (job.serveStartingProfile === name) return job.serveStarting;
    await job.serveStarting.catch(() => {});
  }
  ...
  // rather than recomputing the preferred one.
  if (running && (job.serveProfile || null) === name) return servedLinks(job, profile);
  // Claimed before anything awaits: the stop below takes seconds, and a click
  // landing in them must find this start in flight, not a free field.
  job.serveStartingProfile = name;
  job.serveStarting = (async () => {
    if (running) {
      const label = (n) => (n ? `profile ${n}` : 'the plain run commands');
      pushEvent(job, 'info', {
        text: `Switching ▶ Run from ${label(job.serveProfile)} to ${label(name)}: stopping the running app first`,
      });
      await stopDevServe(job);
    }
    return startDevServeProc(job, profile, epoch);
  })().finally(() => {
    job.serveStarting = null;
    job.serveStartingProfile = null;
  });
...
// yet. It bumps serveEpoch instead, and the start gives up at its next step
// rather than creating a database after the close dropped them, or spawning a
// server nothing will ever kill in a clone handed back to the pool.
function serveCalledOff(job, epoch) {
  if ((job.serveEpoch || 0) === epoch && DEV_OPEN.includes(job.status) && job.workDir) return;
  throw new Error('The session let go of its workspace while ▶ Run was starting');
}

async function startDevServeProc(job, profile, epoch) {
  serveCalledOff(job, epoch);
...
function killDevServe(job) {
  job.serveEpoch = (job.serveEpoch || 0) + 1;
  dropServeProc(job);
}

function dropServeProc(job) {
...
async function stopDevServe(job) {
  ...
  const port = job.appPort;
  // Not killDevServe: that would call off the very start this stop is for.
  dropServeProc(job);
  bus.emit('job', publicJob(job));
