...
export function implementFeedbackPrompt({
  ...
  const declared = findings.length
    ? [
        ...
        ...(note ? ['', `${who} adds: ${String(note).trim()}`] : []),
      ].join('\n')
    : '';
  return renderTemplate(
    'implementFeedback',
