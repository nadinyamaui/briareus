...
// turn runs.
//
// With run profiles, the one the user was shown (or the default) rides along:
// its env as exports and its commands ahead of the run commands, so the agent
// serves the same configuration. Its tenants are named on the .localhost
// hostnames rather than the published ones: the agent's browser runs on this
// machine and cannot get past the tunnel's Access login, while Chromium sends
// every *.localhost name to the loopback on its own. {database} is filled in
// when the caller knows the session (the QA loop's run); a board-started run
// is composed before its session exists, so the agent is told what it means.
/**
 * @param {any} project
...
function runCommandBlock(project, portHint, { profile: wanted = null, database = null } = {}) {
  ...
  // ▶ Run refuses these run commands (devServeRecipe), so the agent is not
  // handed a literal {host:<tenant>} to register or open either.
...
export function implementFeedbackPrompt({
  ...
  // A triaged round is a decided list, not a starting point: whoever ruled on
  // it (the user at the ⚑ Findings screen, or an orchestrator they told to)
  // left the rest out on purpose, and its threads on the pull request are not
  // this session's to act on however open they look. The template's steps
  // that speak of assessing which findings need a fix are told off here, since
  // that judgment was the triage's and is not this session's to remake. `by`
  // is a lowercase noun phrase ("the user", "the orchestrator"), capitalised
  // only where it opens a sentence.
