...
export async function saveTurnUsage(row) {
  ...
  await p.query({
    sql: `INSERT INTO \`turn_usage\`
       (\`project_id\`, \`job_id\`, \`repo\`, \`provider\`, \`model\`, \`activity\`, \`account_id\`, \`account_label\`, \`session_title\`, \`input_tokens\`, \`output_tokens\`, \`cost_usd\`, \`duration_ms\`, \`at\`)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    values: [
      row.projectId ?? null,
      row.jobId,
...
// Every turn belonging to the sessions currently restored in memory. This is
// the session list's counterpart to loadAllTurnUsage: it has no date window,
// because a session's header describes its lifetime spend, and one indexed
// query covers the whole list instead of doing one query per conversation.
...
// share formula is linear within one provider/model rate card, so these sums
// are exactly equivalent to handing it every provider-priced ledger row while
// the amount returned stays proportional to the models in use, not to history.
