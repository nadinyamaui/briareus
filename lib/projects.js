...
// What the project's ⌕ Code review was set up to run on, or null for "the
// session's own". Same shape and same rule as stepRuntime: a model and an
// effort name options of that provider's list, so they only count together
// with a provider.
...
function normalizeProject(input, existing = null) {
  ...
  // Kept as typed, but only once it reads: a profile that fails to parse would
  // otherwise surface as a ▶ Run that quietly serves the plain setup. Only the
  // end is trimmed, so a parse error's line number is the textarea's.
  if (has('runProfiles')) {
...
// {token} substitution lives with the run profiles, whose placeholders it
// fills; re-exported here, where the run commands' callers have always found it.
