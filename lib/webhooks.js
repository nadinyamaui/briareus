...
import { syncSessionsOn, noteWebhookDelivery } from './jobs.js';
...
export function webhookRouter() {
  ...
  router.post('/github', async (req, res) => {
    ...
    // A signed delivery is the proof the hook reaches this install, which is
    // what lets the sync timer slow down for the repository (lib/jobs.js).
...
async function handleGithubEvent(event, payload) {
  ...
  switch (event) {
    ...
    // so these ask for the checks to be read whatever the pull request says.
    case 'check_suite':
      syncSessionsOn(repo, (payload.check_suite || {}).head_branch, null, { checks: true });
      break;
    case 'check_run':
      syncSessionsOn(repo, ((payload.check_run || {}).check_suite || {}).head_branch, null, { checks: true });
      break;
    case 'status':
      // A commit status names no branch either; the branches it belongs to are
      // in the payload when GitHub can work them out.
      for (const b of payload.branches || []) syncSessionsOn(repo, b.name, null, { checks: true });
      break;
    case 'push':
